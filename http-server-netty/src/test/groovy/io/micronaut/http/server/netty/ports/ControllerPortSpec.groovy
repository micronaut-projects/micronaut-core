...
class ControllerPortSpec extends Specification {
    ...
    void "test custom controller port"() {
        ...
        def client = embeddedServer.applicationContext.createBean(ReactorHttpClient, new URL("http://localhost:$customPort"))
        def client2 = embeddedServer.applicationContext.createBean(ReactorHttpClient, embeddedServer.getURL())
