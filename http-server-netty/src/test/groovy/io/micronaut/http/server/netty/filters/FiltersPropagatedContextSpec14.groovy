...
class FiltersPropagatedContextSpec14 extends Specification {

    void "test filter context propagation"() {
        given:
            EmbeddedServer server = ApplicationContext.run(EmbeddedServer, ['spec.name': FiltersPropagatedContextSpec14.simpleName])
    ...
    @Requires(property = 'spec.name', value = 'FiltersPropagatedContextSpec14')
    @Client("/")
    ...
    static class Filter1 implements Ordered {
        ...
        @RequestFilter
        Publisher<HttpResponse<?>> myFilter(HttpRequest<?> request, FilterContinuation<Publisher<HttpResponse<?>>> continuation, MutablePropagatedContext context) {
            context.add(new MyContext())
