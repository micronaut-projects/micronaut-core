...
class FiltersPropagatedContextSpec13 extends Specification {

    void "test filter context propagation"() {
        given:
            EmbeddedServer server = ApplicationContext.run(EmbeddedServer, ['spec.name': FiltersPropagatedContextSpec13.simpleName])
    ...
    @Requires(property = 'spec.name', value = 'FiltersPropagatedContextSpec13')
    @Client("/")
    ...
    static class Filter1 implements Ordered {
        ...
        @RequestFilter
        Publisher<MutableHttpResponse<?>> myFilter(HttpRequest<?> request, FilterContinuation<Publisher<MutableHttpResponse<?>>> continuation) {
            return PropagatedContext.getOrEmpty()
                .plus(new MyContext())
                .propagateCall(() -> continuation.request(request).proceed())
        }
    ...
    @ServerFilter("/filters**")
    @Requires(property = 'spec.name', value = 'FiltersPropagatedContextSpec13')
    static class Filter2 implements Ordered {

        @RequestFilter
        Flux<HttpResponse<?>> myFilter(HttpRequest<?> request, FilterContinuation<Publisher<HttpResponse<?>>> continuation) {
            return Flux.just(true).flatMap(ignore -> continuation.request(request).proceed())
        }
