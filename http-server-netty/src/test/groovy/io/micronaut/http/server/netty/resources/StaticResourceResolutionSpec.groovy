...
class StaticResourceResolutionSpec extends AbstractMicronautSpec {
    ...
    void "test resources with configured mapping automatically resolves index.html in path"() {
        given:
        EmbeddedServer embeddedServer = ApplicationContext.run(EmbeddedServer, [
                'micronaut.router.static-resources.default.paths': ['classpath:foo'],
                'micronaut.router.static-resources.default.enabled': true,
                'micronaut.router.static-resources.default.mapping': '/static/**'], 'test')
        RxHttpClient rxClient = embeddedServer.applicationContext.createBean(RxHttpClient, embeddedServer.getURL())
