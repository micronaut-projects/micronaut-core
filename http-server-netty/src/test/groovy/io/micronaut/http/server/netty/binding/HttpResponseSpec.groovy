...
class HttpResponseSpec extends AbstractMicronautSpec {
    ...
    void "test custom HTTP response for java action #action"() {
        ...
        "ok-with-body-object" | HttpStatus.OK                 | '{"age":10,"name":"blah"}' | defaultHeaders + ['content-length': '24', 'content-type': 'application/json']
        "status"              | HttpStatus.MOVED_PERMANENTLY  | null                       | ['content-length': '0']
        "created-body"        | HttpStatus.CREATED            | '{"age":10,"name":"blah"}' | defaultHeaders + ['content-length': '24', 'content-type': 'application/json']
        "created-uri"         | HttpStatus.CREATED            | null                       | ['content-length': '0', 'location': 'http://test.com']
        "created-body-uri"    | HttpStatus.CREATED            | '{"age":10,"name":"blah"}' | defaultHeaders + ['content-length': '24', 'content-type': 'application/json'] + ['location': 'http://test.com']
