...
class FiltersPropagatedContextSpec12 extends Specification {

    void "test filter context propagation"() {
        given:
            EmbeddedServer server = ApplicationContext.run(EmbeddedServer, ['spec.name': FiltersPropagatedContextSpec12.simpleName])
    ...
    @Requires(property = 'spec.name', value = 'FiltersPropagatedContextSpec12')
    @Client("/")
    ...
    static class Filter1 implements HttpServerFilter {
        ...
        Publisher<MutableHttpResponse<?>> doFilter(HttpRequest<?> request, ServerFilterChain chain) {
            return Flux.just(true).flatMap(ignore -> chain.proceed(request))
        }
