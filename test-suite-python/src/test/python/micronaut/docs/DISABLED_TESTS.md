...
| `annotation/PetControllerSpec.py::testPostPetValidation` | GraalPy exception matching fails while catching the Java validation exception. |
...
| `factories/nullable/EngineSpec.py` | `GR-71643`: GraalPy Java exception interop issue. |
...
| `httpclientexceptionbody/BindHttpClientExceptionBodySpec.py::verifyBindErrorIsThrown` | GraalPy exception matching fails while catching the HTTP client response bind error. |
...
| `respondingnotfound/BooksSpec.py::test_returning_null_returns_404` | Python `None` controller results are not mapped to the Java-equivalent 404 response. |
| `respondingnotfound/BooksSpec.py::test_returning_mono_empty_returns_404` | Python cannot express `@SingleResult` import from `io.micronaut.core.async.annotation` yet. |
...
| `server/json/PersonControllerSpec.py::test_person_not_found` | Python `Publisher` empty results are not mapped to the Java-equivalent 404 response. |
...
| `annotation/headers/PetClient.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
| `annotation/retry/PetClient.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
| `annotation/retry/PetFallback.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
| `client/versioning/HelloClient.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
...
| `http/server/reactive/PersonController.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
| `http/server/response/textplain/TextPlainController.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
| `http/server/netty/websocket/ChatClientWebSocket.py` | Re-enable `@ClientWebSocket` when Python introduction advice can proxy Python classes; re-enable `@SingleResult` when reserved-word imports are supported. |
| `http/server/netty/websocket/PojoChatClientWebSocket.py` | Re-enable `@ClientWebSocket` when Python introduction advice can proxy Python classes; re-enable `@SingleResult` when reserved-word imports are supported. |
| `propagation/reactor/HelloController.py` | Re-enable the direct `ReactorPropagation` import when Python can express imports from `io.micronaut.core.async.propagation` without a reserved-word module path. |
| `respondingnotfound/BooksController.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
| `server/body/MessageController.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
| `server/json/PersonController.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
| `server/intro/HelloClient.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
...
| `server/upload/UploadController.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
| `server/upload/WholeBodyUploadController.py` | Re-enable `@SingleResult` when Python can express imports from `io.micronaut.core.async.annotation` without a reserved-word module path. |
