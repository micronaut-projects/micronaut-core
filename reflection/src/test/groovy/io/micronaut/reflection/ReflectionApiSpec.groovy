...
class ReflectionApiSpec extends Specification {
    ...
    void "a container asks whether a type can be a bean before it binds it"() {
        expect: "the types a definition can be built for"
        ReflectionBeanDefinition.isDefinable(Warehouse)
        ReflectionBeanDefinition.isDefinable(Delegating)
        ReflectionBeanDefinition.isDefinable(Factories)

        and: "and the ones it cannot, which of(Class) reports by throwing"
        !ReflectionBeanDefinition.isDefinable(Reservable)
        !ReflectionBeanDefinition.isDefinable(AbstractReservation)
        !ReflectionBeanDefinition.isDefinable(Level)
        !ReflectionBeanDefinition.isDefinable(int)
        !ReflectionBeanDefinition.isDefinable(String[])
        !ReflectionBeanDefinition.isDefinable(Locked)

        and: "the answer agrees with what building one does"
        [Warehouse, Delegating, Factories, Reservable, AbstractReservation, Level, Locked].every { Class<?> type ->
            boolean built
            try {
            ...
            built == ReflectionBeanDefinition.isDefinable(type)
        }
    }
