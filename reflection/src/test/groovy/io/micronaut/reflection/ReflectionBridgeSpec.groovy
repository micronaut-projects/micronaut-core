...
class ReflectionBridgeSpec extends Specification {
    ...
    void "the metadata of a class has its declared annotations, their stereotypes, the repeated ones and the inherited ones"() {
        when:
        def metadata = ReflectionAnnotations.metadataOf(Book)
    ...
    void "a repeated annotation keeps every occurrence with its values and its defaults"() {
        when:
        def metadata = ReflectionAnnotations.metadataOf(Book.getDeclaredField("title"))
    ...
    void "class, enum and nested annotation members are stored as the generated metadata stores them"() {
        when:
        def metadata = ReflectionAnnotations.metadataOf(Book.getMethod("discount", double))
    ...
    void "the argument of a field carries the type-use annotations of its type arguments"() {
        when:
        def argument = ReflectionArguments.of(Book.getDeclaredField("tags"))
    ...
    void "the arguments of a method carry the annotations of the parameters"() {
        when:
        def arguments = ReflectionArguments.argumentsOf(Book.getMethod("discount", double))
    ...
    void "a reflective introspection exposes the properties, the constructor and the methods"() {
        ...
        // a bean method carries the annotations of its bean type too, the way a generated one does: the @Tag the
        // method declares, the one Book declares and the two @Composed composes
        introspection.beanMethods.find { it.name == "discount" }.annotationMetadata.getAnnotationValuesByType(Tag)*.stringValue()*.get().toSorted() ==
            ["disc", "entity", "first", "second"]
    }
