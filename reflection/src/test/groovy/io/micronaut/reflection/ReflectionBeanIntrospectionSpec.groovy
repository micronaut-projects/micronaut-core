...
class ReflectionBeanIntrospectionSpec extends Specification {
    ...
    void "a type asking for no index holds no property, as a generated description of it holds none"() {
        given: "a type carrying the annotations but asking for no index"
        def introspection = ReflectionBeanIntrospection.of(Shadowed)

        expect: "the properties do carry them"
        introspection.getRequiredProperty("marked", String).annotationMetadata.hasStereotype(Hidden)
        introspection.getRequiredProperty("value", String).annotationMetadata.hasStereotype(Stereo)

        and: "and the index holds nothing: the processor builds one for the annotations Introspected#indexed names and for no other"
        introspection.getIndexedProperties(Stereo).isEmpty()
        introspection.getIndexedProperties(Hidden).isEmpty()
        introspection.getIndexedProperties(Shape).isEmpty()
        introspection.getIndexedProperty(Hidden, "indexed").empty
