...
class AnnotatePropertySpec extends AbstractBeanDefinitionSpec {

    def setup() {
        System.setProperty(TypeElementVisitorStart.ELEMENT_VISITORS_PROPERTY, AnnotatePropertyVisitor.name)
    }
    ...
    static class AnnotatePropertyVisitor implements TypeElementVisitor<Object, Object> {
        ...
        void visitClass(ClassElement classElement, VisitorContext context) {
            if (classElement.getSimpleName().startsWith("AnnotatePropertyBean")) {
                ...
                assert property1.hasAnnotation(MyAnnotation.class.name)
