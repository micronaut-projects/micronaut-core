...
public class TypeElementVisitorProcessor extends AbstractInjectAnnotationProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        if (!loadedVisitors.isEmpty()) {
            ...
            // This processor will be called 3 times, first for when the processor processes the sources
            // then again for the classes generated by this processor (such as additional bean introspections)
            // and then again when processing is over. We only want to process the first instance therefore
            // the variable 'processed' is set to true after the first round and set back to false after the
            // last round (effectively roundEnv.processingOver() returning true)
            if (!processed) {
                for (LoadedVisitor loadedVisitor : loadedVisitors) {
                    try {
