...
class BeanProviderInjectionSpec extends Specification {
    @AutoCleanup
    ApplicationContext context = ApplicationContext.run()

    void 'test proper provider lookup'() {
        when:
            BeanProviderInjectee bean = context.getBean(BeanProviderInjectee)
        then:
            ...
            ForkJoinPool.commonPool()
                    .invokeAll(
                            IntStream.range(0, 1000)
                                    .mapToObj(
                                            idx -> () -> {
                                                def bean = context.getBean(BeanProviderInjectee)
                                                bean.showWhatIsInjected()
                                                return true
                                            })
                                    .collect(Collectors.toList()))
    ...
    void 'test concurrency3'() {
        when:
            BeanProviderInjectee bean1 = context.getBean(BeanProviderInjectee)
            ForkJoinPool.commonPool()
                    .invokeAll(
                            IntStream.range(0, 1000)
                                    .mapToObj(
                                            idx -> () -> {
                                                def bean2 = context.getBean(BeanProviderInjectee)
                                                assert bean1 == bean2
