...
/**
 * Java port of ReplacesRepeatableAnnotationSpec.
 */
class ReplacesRepeatableAnnotationTest extends AbstractTypeElementTest {

    @Override
    protected java.util.Collection<TypeElementVisitor<?, ?>> getLocalTypeElementVisitors() {
        return java.util.List.of(new ReplacesRepeatableTypeElementVisitor());
    }

    @Test
    void testReplaceSimpleAnnotation() {
        BeanDefinition<?> definition = buildBeanDefinition("addann.ReplaceAnnotationsTo", """
            package addann;
    ...
    static final class ReplacesRepeatableTypeElementVisitor implements TypeElementVisitor<Object, Object> {
        @Override
        public void visitClass(ClassElement element, VisitorContext context) {
            if ("ReplaceAnnotationsTo".equals(element.getSimpleName())) {
                final var indexes = Stream.concat(
                    getIndexes(element.getAnnotationMetadata()),
                    element.getFields().stream().flatMap(fe -> getIndexes(fe.getAnnotationMetadata()))
                ).toList();

                // Re-annotate with container including all gathered values
                element.annotate("io.micronaut.annotation.mapping.MyRequirements", builder ->
                    builder.values(indexes.toArray(new AnnotationValue[0]))
                );
            }
        }

        private Stream<AnnotationValue<?>> getIndexes(AnnotationMetadata am) {
            var container = am.getAnnotation("io.micronaut.annotation.mapping.MyRequirements");
            if (container == null || container.getAnnotations("value").isEmpty()) {
                throw new IllegalStateException();
            }
            // Recreate the values to avoid typed dependency
            return container.getAnnotations("value").stream().map(av -> {
