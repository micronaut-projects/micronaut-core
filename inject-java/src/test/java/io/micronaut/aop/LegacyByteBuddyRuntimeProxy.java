...
@NullMarked
/**
 * The 5.2 shape of the ByteBuddy creator: it reads each intercepted method's own interceptors and never asks the
 * definition for the target of a call. Kept for the regression that such a creator still applies the singletons.
 */
public class LegacyByteBuddyRuntimeProxy implements RuntimeProxyCreator {
    ...
    public <T> T createProxy(RuntimeProxyDefinition<T> proxyDefinition) {
        ...
        builder = builder.name(targetType.getName() + "$LegacyByteBuddyProxy");
