...
def ensure_non_empty_bodies(tree: ast.AST) -> None:
    ...
    only statement was an import that became a generated decorator, or the ``finally:`` of a ``try``
    (which must keep a handler or a final body), so the tree still unparses and compiles.
    """
    for node in ast.walk(tree):
        ...
        if isinstance(node, (ast.Try, ast.TryStar)) and not node.handlers and not node.finalbody:
            node.finalbody = [ast.copy_location(ast.Pass(), node)]
...
# The top-level segments of import modules the compiler reserves for Java: an import from such a module that
# names no class on the compile classpath cannot be satisfied by any Python module at run time.
JAVA_IMPORT_NAMESPACES = frozenset({"micronaut", "io", "java", "javax", "jakarta"})
...
class MicronautTransformer(ast.NodeTransformer):
    ...
    def __init__(self, callback_get_class_element, callback_get_class_elements, strip_java_interface_bases=False,
        ...
        Args:
            ...
            strip_java_interface_bases: Whether to produce the runtime form of the module
        ...
        self.package_name = package_name or ''
        self.source_root = source_root or ''
        self.python_source_dirs = [str(source_dir) for source_dir in python_source_dirs if source_dir] if python_source_dirs else []
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        ...
        # The bindings of the imported Java classes take the place of the import, so an import guarded by
        # ``try:`` / ``except ImportError:`` keeps its guard; the generated decorators are hoisted to the module
        bindings = []
        for alias in node.names:
            if alias.name == '*':
                ...
                if binding:
                    transformed_any = True
                    if isinstance(binding, ast.stmt):
                        bindings.append(ast.copy_location(binding, node))
        ...
        return [node] + bindings if bindings else node
...
'''
    ...
    def _handle_specific_import(self, original_module_name: str, transformed_module_name: str, alias):
        """
        ...
        Returns the ``java.type()`` assignment replacing the import of a Java class, True for an import
        replaced by a generated decorator, and False if the import was not transformed.
        """
        ...
        return ast.parse(f"{variable_name} = java.type('{class_element.getName()}')").body[0]
    ...
    def _check_unresolved_java_import(self, python_module_name: str, java_module_name: str, import_name: str):
        """
        Report a from-import of a name that has to be a Java class but is not on the compile classpath, instead
        of leaving it to resolve to ``Object`` in the generated signatures. An import is a Java import when its
        module is a Java package with classes on the classpath, or lies in a namespace reserved for Java; a module
        of the project's Python sources or an importable Python module (the standard library) is never one.
        """
        if self._resolves_to_java_class(java_module_name, import_name):
            # a known class: the import was handled before (a decorator generated by an earlier import, say)
            return
        if self._resolves_to_compiled_python_class(python_module_name, import_name):
            # the generated bridge of a Python class compiled by another source root or into a library
            return
        if self._is_java_package(f"{java_module_name}.{import_name}"):
            # a package imported as a name: from jakarta import inject
            return
        if self.callback_get_class_element(java_module_name) is not None:
            # the module names a Java type (from a.b.Outer import Outer): not a package import
            return
        if self._is_project_python_module(python_module_name) or self._is_python_module(python_module_name):
            return
    ...
    def _resolves_to_compiled_python_class(self, python_module_name: str, import_name: str) -> bool:
        """
        Whether the import names a Python class compiled earlier (by another source root of the project, or
        into a library) whose generated bridge is on the class path: the class of a module is generated into
        the package of the module.
        """
        parent_name = python_module_name.rsplit('.', 1)[0] if '.' in python_module_name else 'python'
        for candidate in (f"{python_module_name}.{import_name}", f"{parent_name}.{import_name}"):
            class_element = self.callback_get_class_element(candidate)
            if class_element is not None and _JavaTypes.isPythonClass(class_element):
                return True
        return False

    def _resolves_to_java_class(self, java_module_name: str, import_name: str) -> bool:
        for name in (import_name, self._to_python_case(import_name)):
            if self.callback_get_class_element(f"{java_module_name}.{name}") is not None:
                return True
        return False

    def _java_type_name_of_import(self, java_module_name: str, import_name: str) -> str:
        if not java_module_name.startswith('io.') and java_module_name.startswith('micronaut.'):
            java_module_name = f"io.{java_module_name}"
        return f"{java_module_name}.{import_name}"

    def _is_project_python_module(self, module_name: str) -> bool:
        """
        Whether the module is a Python module or package of the project's source directories.
        """
        module_path = os.path.join(*module_name.split('.'))
        for source_dir in self.python_source_dirs:
            if os.path.isdir(os.path.join(source_dir, module_path)) or os.path.isfile(os.path.join(source_dir, f"{module_path}.py")):
                return True
        return False
    ...
    def _is_java_package(self, java_module_name: str) -> bool:
        """
        Whether the module names a Java package with classes on the compile classpath.
        """
        class_elements = self.callback_get_class_elements(java_module_name)
        return class_elements is not None and len(class_elements) > 0
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom):
        ...
        for alias in node.names:
            ...
            if alias.name == '*':
                ...
                self.java_runtime_names.update(self._star_imported_class_names(java_module))
                continue
