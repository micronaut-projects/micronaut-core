...
/**
 * Typed locators and the located routes of targets of a type, whose handlers receive the
 * target: {@link io.micronaut.web.router.builder.LocatedRoutes}.
 */
@SuppressWarnings({
...
public class HandlerRouteTypedLocatorTest {
    ...
    @Test
    void theRoutesFunctionReceivesTheTypedTarget() throws IOException {
        try (ServerUnderTest server = server()) {
    ...
    static class TypedLocatorRoutes implements HttpRoutes {
        ...
        TypedLocatorRoutes(@Named(TaskExecutors.IO) ExecutorService executor) {
            this.executor = executor;
        }

        @Override
        public void routes(HttpRouteBuilder routes) {
            LocatedRoutes<Shop> shops = TckLocatedRoutes.of(Shop.class, shop -> {
            ...
            }, shops); // one set of routes for every located shop

            LocatedRoutes<?> bigShops = TckLocatedRoutes.of(Argument.of(Shop.class), shop ->
            ...
            LocatedRoutes<?>[] folders = new LocatedRoutes<?>[1];
            folders[0] = TckLocatedRoutes.of(Folder.class, folder -> {
