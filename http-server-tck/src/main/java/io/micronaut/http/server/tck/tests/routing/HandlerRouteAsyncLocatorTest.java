...
public class HandlerRouteAsyncLocatorTest {
    ...
    static class AsyncLocatorRoutes implements HttpRoutes {
        ...
        AsyncLocatorRoutes(@Named(TaskExecutors.IO) ExecutorService executor) {
            this.executor = executor;
        }

        @Override
        public void routes(HttpRouteBuilder routes) {
            LocatedRoutes<?> items = TckLocatedRoutes.of(located -> located.GET("/items/{item}", (request, pathVariables) ->
                text(HttpStatus.OK, "order " + LocatedRoutes.locatedTarget(pathVariables, Order.class).id() + " item " + pathVariables.getInt("item"))));
            ...
            LocatedRoutes<?>[] tree = new LocatedRoutes<?>[1];
            tree[0] = TckLocatedRoutes.of(node -> {
                node.GET("/leaf", (request, pathVariables) ->
                    text(HttpStatus.OK, "leaf of " + LocatedRoutes.locatedTarget(pathVariables) + ", root " + pathVariables.getString("root")));
            ...
            LocatedRoutes<?> shows = TckLocatedRoutes.of(located -> located.GET("/show", (request, pathVariables) ->
                text(HttpStatus.OK, "locator=" + LocatedRoutes.locatedTarget(pathVariables) + " handler=" + trace())));
