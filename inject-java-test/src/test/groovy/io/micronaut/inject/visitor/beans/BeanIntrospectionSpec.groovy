...
''')
    ...
    void "test property members are not generated by default"() {
        given:
...
''')
    ...
    void "test a property lists the field and the getter as separate members"() {
        given:
        def introspection = buildBeanIntrospection('test.Person', '''
package test;
...
class Person {
    ...
    @Size(min = 5) public String getName() { return "Bob"; }
    public void setName(String name) { this.name = name; }
}
''')
        def property = introspection.getProperty("name").get()
        def members = property.members

        expect: "the property itself still exposes the merged view read through the getter"
        property.annotationMetadata.hasAnnotation(NotNull)
        property.annotationMetadata.hasAnnotation(Size)
        property.get(introspection.instantiate()) == "Bob"

        and: "the field, the getter and the setter are listed separately"
        members.size() == 3
        members*.name == ["name", "getName", "setName"]
        members*.elementType == [ElementType.FIELD, ElementType.METHOD, ElementType.METHOD]
        members.every { it.declaringType == introspection.beanType }

        and: "each member carries only its own annotations"
        members[0].annotationMetadata.hasAnnotation(NotNull)
        !members[0].annotationMetadata.hasAnnotation(Size)
        !members[1].annotationMetadata.hasAnnotation(NotNull)
        members[1].annotationMetadata.hasAnnotation(Size)
        !members[2].annotationMetadata.hasAnnotation(NotNull)
        !members[2].annotationMetadata.hasAnnotation(Size)

        and: "the field is read directly rather than through the getter"
        members[0].readable
        members[1].readable
        !members[2].readable
        members[0].read(introspection.instantiate()) == "Billy"
        members[1].read(introspection.instantiate()) == "Bob"

        when:
        members[2].read(introspection.instantiate())

        then:
        thrown(UnsupportedOperationException)
    }

    void "test property members expose their own argument"() {
        given:
...
class Holder {
    ...
    public Iterable<@NotNull String> getValues() { return values; }
    public void setValues(Set<String> values) { this.values = values; }
}
''')
        def members = introspection.getProperty("values").get().members

        expect: "the setter is dropped because its type differs from the getter"
        members.size() == 2

        and: "the field keeps its own type while the getter keeps its own type and type annotations"
        members[0].asArgument().type == Set
        members[1].asArgument().type == Iterable
        members[1].asArgument().typeParameters[0].annotationMetadata.hasAnnotation(NotNull)
    }

    void "test property members of a record"() {
        given:
        def introspection = buildBeanIntrospection('test.Person', '''
package test;
...
''')
        ...
        members*.elementType == [ElementType.FIELD, ElementType.METHOD]
        members.every { it.annotationMetadata.hasAnnotation(NotNull) }
        members.every { it.readable }
        members*.read(introspection.instantiate("Billy")) == ["Billy", "Billy"]
    }

    void "test property members report the declaring type"() {
        given:
