...
class DefaultsControllerSpec : StringSpec() {

    val embeddedServer = autoClose(
        ApplicationContext.run(EmbeddedServer::class.java, mapOf("spec.name" to "defaults"))
    )
    ...
    init {
        "test echo response"() {
            val response1 = client.toBlocking().retrieve(
                HttpRequest.POST("/defaults/echo", "My Text")
        ...
        "test echo reactive response"() {
            val response1 = client.toBlocking().retrieve(
                HttpRequest.POST("/defaults/echo-publisher", "My Text")
                    .header("MYHEADER", "abc123")
                    .contentType(MediaType.TEXT_PLAIN_TYPE), String::class.java
            )
