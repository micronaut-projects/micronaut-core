...
import static org.particleframework.core.beans.Introspector.decapitalize;
...
class StandardBeanInfo extends SimpleBeanInfo {
    ...
    StandardBeanInfo(Class<?> beanClass, BeanInfo explicitBeanInfo, Class<?> stopClass)
        ...
        if (properties == null) {
            properties = introspectProperties(stopClass, methods);
        }

        if (events == null) {
            events = introspectEvents(methods);
        }
    }
    ...
    private static HashMap<String, PropertyDescriptor> internalAsMap(
        ...
        for (PropertyDescriptor propertyDesc : propertyDescs) {
            map.put(propertyDesc.getName(), propertyDesc);
        }
        return map;
    }
    ...
    private static HashMap<String, MethodDescriptor> internalAsMap(
        ...
        for (MethodDescriptor theDesc : theDescs) {
            String qualifiedName = getQualifiedName(theDesc.getMethod());
            map.put(qualifiedName, theDesc);
        }
        return map;
    }
    ...
    private static HashMap<String, EventSetDescriptor> internalAsMap(
        ...
        for (EventSetDescriptor theDesc : theDescs) {
            map.put(theDesc.getName(), theDesc);
        }
        return map;
    }

    private static String getQualifiedName(Method method) {
        StringBuilder qualifiedName = new StringBuilder(method.getName());
        Class<?>[] paramTypes = method.getParameterTypes();
        if (paramTypes != null) {
            for (Class<?> paramType : paramTypes) {
                qualifiedName.append("_").append(paramType.getName()); //$NON-NLS-1$
            }
        }
    ...
    private PropertyDescriptor[] introspectProperties(Class<?> stopClass, MethodDescriptor[] methodDescriptors)
            throws IntrospectionException {
        ...
        for (MethodDescriptor methodDescriptor : methodDescriptors) {
            introspectGet(methodDescriptor.getMethod(), propertyTable);
            introspectSet(methodDescriptor.getMethod(), propertyTable);
        }
     ...
     */
    @SuppressWarnings("unchecked")
    private EventSetDescriptor[] introspectEvents(MethodDescriptor[] theMethods) throws IntrospectionException {
        if (theMethods == null)
            return null;
        ...
        for (MethodDescriptor theMethod : theMethods) {
            introspectListenerMethods(PREFIX_ADD, theMethod.getMethod(),
                    eventTable);
            introspectListenerMethods(PREFIX_REMOVE, theMethod.getMethod(),
                    eventTable);
            introspectGetListenerMethods(theMethod.getMethod(), eventTable);
        }
