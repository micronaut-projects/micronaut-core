...
public abstract class AbstractBeanProperty<B, P> implements BeanProperty<B, P> {
    Class[] PROPERTYNAME_ANNOTATIONS = new Class[] { JsonProperty.class };
    ...
    private String resolvedName(String name) {
        for (Class propertyNameAnnotation : PROPERTYNAME_ANNOTATIONS) {
            Optional<AnnotationValue> opt = annotationMetadata.findAnnotation(propertyNameAnnotation);
            if (opt.isPresent()) {
                AnnotationValue jsonProperty = opt.get();
                if (jsonProperty.stringValue().isPresent()) {
                    return (String) jsonProperty.stringValue().get();
                }
            }
        }
        return name;
