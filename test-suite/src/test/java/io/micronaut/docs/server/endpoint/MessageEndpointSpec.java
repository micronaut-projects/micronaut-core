...
public class MessageEndpointSpec {
    ...
    public void testReadMessageEndpoint() {
        ...
        HttpResponse<String> response = rxClient.exchange("/message", String.class).blockFirst();
    ...
    public void testWriteMessageEndpoint() {
        ...
        response = rxClient.exchange("/message", String.class).blockFirst();
    ...
    public void testDeleteMessageEndpoint() {
        ...
        EmbeddedServer server = ApplicationContext.run(EmbeddedServer.class, map);
        ReactorHttpClient rxClient = server.getApplicationContext().createBean(ReactorHttpClient.class, server.getURL());
        HttpResponse<String> response = rxClient.exchange(HttpRequest.DELETE("/message"), String.class).blockFirst();

        assertEquals(HttpStatus.OK.getCode(), response.code());
        assertEquals("Message deleted", response.body());

        try {
            rxClient.exchange("/message", String.class).blockFirst();
