...
created with it, as dependents of its registration, and every later interception point takes them from there.
...
An interceptor bound for `AROUND` alone is not resolved while the target is created; the proxy creates it for the
target on the first call and hands it to the target's registration, so it is destroyed with the target as well.
