...
NOTE: A lifecycle interception runs once per callback, so a bean with several `@PostConstruct` methods in its class
hierarchy is intercepted once for each of them, with `getExecutableMethod()` naming the callback. See
...
. `POST_CONSTRUCT` interception runs around each of the bean's `@PostConstruct` methods, superclass methods first,
  or once for the bean when it declares none.
. `AROUND` or `INTRODUCTION` interception runs on each method call, for as long as the bean lives.
. `PRE_DESTROY` interception runs around each of the bean's `@PreDestroy` methods, or once for the bean when it
  declares none.
