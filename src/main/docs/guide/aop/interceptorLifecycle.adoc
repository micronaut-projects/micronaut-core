...
The proxy is also constructed with an instance of each interceptor bound to its methods. For a non-singleton
interceptor that instance does not intercept a target created through the proxy; it is destroyed with the proxy.
