...
|`@Around(proxyTarget = true)`, including a ann:aop.runtime.RuntimeProxy[]
|The target's registration in the context; the proxy invokes the target with them
|construct, methods, post-construct, pre-destroy

|A scoped proxy, that is `@Around(proxyTarget = true, lazy = true)`, such as ann:runtime.context.scope.ThreadLocal[], `@RequestScope` or ann:runtime.context.scope.Refreshable[]
|The registration of each target in the context; on every call the proxy selects the instances of the target it resolved
|construct, methods, post-construct, pre-destroy, for each target
...
With `proxyTarget = true` the intercepted bean is the target, not the proxy. A scoped proxy is one instance that
stands for a different target in each scope: one per thread for `@ThreadLocal`, one per request for `@RequestScope`.
Each of those targets is an intercepted bean of its own, so each gets its own instance of a non-singleton interceptor.
The instance is created with the target, calls made through the proxy while that target is current are intercepted by
it, and it is destroyed when the scope destroys the target. Two requests calling the same proxy therefore never share
the state of a `@Prototype` interceptor, while a singleton interceptor is still one instance for all of them.

The proxy itself holds only the singleton interceptors bound to its methods and creates no instance of a
non-singleton one, which is why constructing a scoped proxy creates none.
...
behave as they did before, resolving an interceptor per interception point. The same applies to a proxy with
`proxyTarget = true` compiled before 5.2.1, which intercepts the methods of every target with the instances it was
constructed with.
