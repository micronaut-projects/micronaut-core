...
context keeps for the bean, as its dependents, which is the bean's dependent scope.
...
TIP: A generated proxy resolves its interceptors in the bean's dependent scope, and a proxy that fronts a separate
target selects that target's interceptors from its registration. After upgrading, recompile intercepted classes so
