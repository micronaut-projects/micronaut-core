...
An interceptor whose position depends on the interception point implements api:aop.InterceptionPointOrdered[]
instead. The registry then places it by the order it reports for the method or constructor being intercepted, so one
interceptor bean can run before other advice on one method and after it on another. This is what an interceptor that
runs a chain of its own needs, when the chain bound to an element declares where in the surrounding advice it belongs:

[source,java]
----
@Prototype
@InterceptorBinding(value = Tracked.class, kind = InterceptorKind.AROUND)
public class ChainInterceptor implements MethodInterceptor<Object, Object>, InterceptionPointOrdered {

    @Override
    public int getOrder() {
        return 2000; // the position the chain takes most often
    }

    @Override
    public int getOrder(Executable<?, ?> method, InterceptorKind kind) {
        return method.getAnnotationMetadata().hasAnnotation(Early.class) ? -200 : getOrder();
    }
...
The order is asked once for each interception point, when its interceptors are resolved, and places the interceptor
as a whole within its kind: around advice always runs before introduction advice, whatever order either reports. For
`POST_CONSTRUCT` and `PRE_DESTROY` the point is the lifecycle event of the bean, so its annotation metadata is the
bean's. `getOrder()` still applies wherever else the interceptor is ordered, and to proxies compiled before
Micronaut 4.3.
