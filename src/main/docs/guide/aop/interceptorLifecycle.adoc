...
The order is asked once for each interception point, when its interceptors are resolved. `getOrder()` still applies
wherever else the interceptor is ordered, and to proxies compiled by a version of Micronaut that did not consult the
point.
