...
NOTE: An object handed to a hot-swappable proxy through `swap` that the context did not create, or a prototype
created for another proxy, has no registration the proxy can find. Such a target is intercepted with instances the
proxy creates once and keeps; a singleton or scoped target swapped in is found and intercepted with its own.
