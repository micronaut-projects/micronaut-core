...
An intercepted method may also be a <<scheduling, scheduled task>>. When the scheduler fires the task, the invocation context carries an api:aop.ScheduledInvocation[] attribute, which `ScheduledInvocation.find(context)` returns; a direct call to the same method from application code carries none. Advice that must only apply to the scheduled firings, such as a Jakarta Interceptors style around-timeout method, checks for it before acting. See <<scheduling, Scheduled Tasks>> for the details.
