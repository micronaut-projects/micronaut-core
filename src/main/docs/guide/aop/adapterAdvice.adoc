...
The presence of the ann:runtime.event.annotation.EventListener[] annotation causes Micronaut to create a new class that implements api:context.event.ApplicationEventListener[] and invokes the `onStartup` method defined in the bean above.
...
NOTE: Micronaut also automatically aligns the generic types for the SAM interface if they are specified.
