...
=== Accessing the Intercepted Callbacks
...
To find out which methods of the bean `proceed()` is about to run, use `getLifecycleCallbacks()`. It returns the
...
public Object intercept(MethodInvocationContext<Object, Object> context) {
    if (context.getKind() == InterceptorKind.POST_CONSTRUCT) {
        for (ExecutableMethod<Object, ?> callback : context.getLifecycleCallbacks()) {
...
The list is empty for a bean without callbacks of the intercepted kind, for a bean compiled by an earlier version of
the framework, and for every other interception kind. The callbacks are not part of the bean's executable methods:
