...
=== Accessing the Intercepted Methods

A `@PostConstruct` or `@PreDestroy` interception stands for the whole phase: one interceptor chain runs per phase, and
`proceed()` invokes every callback of that kind that the bean declares. The api:aop.MethodInvocationContext[] therefore
describes the phase rather than a method of the bean, which is what its interceptor bindings are resolved from.

To find out which methods of the bean `proceed()` is about to run, use `getExecutableMethods()`. For ordinary method
advice it returns the single `getExecutableMethod()`. For lifecycle advice it returns the
`@PostConstruct` or `@PreDestroy` methods of the target as api:inject.ExecutableMethod[] instances, in invocation order,
superclass callbacks first. Each one can be inspected without reflection and, if needed, invoked on `getTarget()`:
...
public Object intercept(MethodInvocationContext<Object, Object> context) {
    if (context.getKind() == InterceptorKind.POST_CONSTRUCT) {
        for (ExecutableMethod<Object, ?> callback : context.getExecutableMethods()) {
            LOG.info("Initializing {} via {}", context.getTarget(), callback.getName()); // <1>
        }
    }
...
<1> Each callback carries the annotation metadata of the method, for example its `@PostConstruct` annotation
<2> `proceed()` still invokes every callback; invoking one from the list runs it in addition

For lifecycle advice, the list is empty when the bean has no callbacks of the intercepted kind or was compiled by an
earlier version of the framework. These callbacks are not part of the bean definition's executable methods:
`BeanDefinition.getExecutableMethods()` does not include them, so executable method processors and adapters are not
affected.
