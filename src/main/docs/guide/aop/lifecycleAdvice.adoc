...
`getExecutableMethod()` describes the phase rather than a method of the bean and `getExecutableMethods()` is empty. The
same applies to a bean compiled by an earlier version of the framework.
