...
The api:core.beans.BeanConstructor[] returned by `getConstructor()` describes the constructor being advised. For a
bean that is also proxied with `@Around` it describes the constructor of the intercepted class, with only the
parameters that class declares, rather than the generated proxy constructor. Its `getTargetConstructor()` resolves
the corresponding `java.lang.reflect.Constructor` once, the way `getTargetMethod()` does for an intercepted method,
and returns `null` when the bean is not created through a constructor of its own type, as for a bean produced by a
factory method.
