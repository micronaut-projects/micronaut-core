...
Environment detection can be disabled by setting the `micronaut.env.deduction` system property or the `MICRONAUT_ENV_DEDUCTION` environment variable to `false`. Additionally, a new method has been added to api:context.ApplicationContextBuilder[] so that this setting can be made when the application context is instantiated.
