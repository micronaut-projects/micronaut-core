...
Framework level dynamic classloading and reflection has been completely removed thus making it easier to get applications running on GraalVM native image and reducing the number of customizations necessary.

==== Build Time Reflection Data

The older `GraalClassLoadingAnalyzer` runtime step has been remove and replaced by build time generation of `reflect.json` for classes that do require it (typically third party libraries).
