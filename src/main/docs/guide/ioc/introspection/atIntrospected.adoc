...
[.lang-kotlin.lang-python]
--
NOTE: Package-level introspection requires a `package-info` declaration, which only Java and Groovy support. In Kotlin and Python, annotate each class with ann:core.annotation.Introspected[], or list the types with the `classes` member as shown in <<introspectedAnnotationOnAConfigurationClass, Use the @Introspected Annotation on a Configuration Class>>.
--
