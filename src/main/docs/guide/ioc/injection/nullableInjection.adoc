...
[.lang-scala]
--
Scala can declare the injected type with a nullable union type such as `Engine | Null`. The framework injects `null` if the bean is unavailable.
--
...
[.lang-scala]
--
<1> Here the constructor argument is declared with the `Engine | Null` union type.
--
[.lang-python]
--
<1> In Python you can use a union type with `None` such as in the example above `Engine | None`.
--
