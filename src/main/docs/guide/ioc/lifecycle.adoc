...
NOTE: For a bean produced by a ann:context.annotation.Factory[], the `preDestroy` value on the producing element is the only one of these that applies: a `@PreDestroy` or `@PostConstruct` method declared by the produced type is not invoked, because the instance is constructed by the factory and is not a managed instance of the produced type. See <<factories, Bean Factories>>.
