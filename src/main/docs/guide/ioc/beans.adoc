A bean is an object whose lifecycle is managed by the Micronaut IoC container. That lifecycle may include creation, execution, and destruction. Micronaut implements the https://javax-inject.github.io/javax-inject/[JSR-330 (jakarta.inject) - Dependency Injection for Java] specification, hence to use Micronaut you simply use the link:{jeeapi}/javax/inject/package-summary.html[annotations provided by jakarta.inject].
