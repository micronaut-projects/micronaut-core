...
|Requires configuration properties bean to be present in case no configProperty specified
|`@Requires(configProperties=Config.class)`

|Requires the specified property of configuration properties class to be present
|`@Requires(configProperties=Config.class, configProperty="enabled")`
|===
