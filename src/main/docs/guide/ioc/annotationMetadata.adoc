...
The Micronaut framework will respect the rules defined in Java's jdk:java.lang.reflect.AnnotatedElement[] API with regards to annotation inheritance:
...
If you do not have control over the annotation, another approach is to use an api:io.micronaut.inject.annotation.processor.AnnotationMapper[]. To create an `AnnotationMapper`, do the following:

* Implement the api:io.micronaut.inject.annotation.processor.AnnotationMapper[] interface
...
NOTE: The example above implements the api:io.micronaut.inject.annotation.processor.NamedAnnotationMapper[] interface which allows for annotations to be mixed with runtime code. To operate against a concrete annotation type, use api:io.micronaut.inject.annotation.processor.TypedAnnotationMapper[] instead, although note it requires the annotation class itself to be on the annotation processor classpath.
