...
As an example the  `AnnotationMapper` that maps the `javax.annotation.security.PermitAll` standard Java annotation to the internal Micronaut `Secured` annotation looks like the following:
...
----
// TODO above link is broken. Source is in an external project. How do we fix?
