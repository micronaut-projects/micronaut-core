...
NOTE: The example above implements the api:io.micronaut.inject.annotation.TypedAnnotationMapper[] interface which requires the annotation class itself to be on the annotation processor classpath. If that is undesirable (such as for projects that mix annotations with runtime code) then you should use api:io.micronaut.inject.annotation.NamedAnnotationMapper[] instead.