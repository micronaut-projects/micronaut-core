...
To publish an event, obtain an instance of api:context.event.ApplicationEventPublisher[] either directly from the context or through dependency injection, and invoke the `publishEvent` method with your event object.
