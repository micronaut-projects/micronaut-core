...
When the `/refresh` endpoint is invoked or a api:runtime.context.scope.RefreshEvent[] is published then the instance is invalidated and a new instance is created the next time the object is requested. For example:
