...
By default, `ConstraintExceptionHandler` create different responses when only one rule is violated and N rules are violated.

One rule violated
----
{
  "message": "Person.age: must be greater than or equal 18",
  "_links": {
    "self": {
      "href": "/persons",
      "templated": false
...
In `one rule violated scenario`, the message field has the rule violation message. In `N rules violated scenario`, the message is default bad request message and the rules violation messages are in `_embedded.errors` field. For some scenarios, if you want to create the same response by putting rule violation message for `one rule violated scenario`, you can configure by setting `jackson.always-serialize-errors-as-list` to `true` in `application.yml`. By default, it is set to `false`.
