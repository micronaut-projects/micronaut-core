...
* Instead of you should use api:validation.validator.constraints.ConstraintValidator[] to define custom constraints, which supports validating annotations as compilation time.

However, Micronaut's implementation includes the following benefits:
