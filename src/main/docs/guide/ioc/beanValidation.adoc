...
.Validation of iterable items

You can validate values of java iterables, like List, Set and Map by defining generic parameters on them.

snippet::io.micronaut.docs.ioc.validation.iterableGenericParameters.BookInfoService[tags="validate-iterables",indent="0"]

<1> List items will be validated with given annotation
<2> Both keys and values of Map will be validated

snippet::io.micronaut.docs.ioc.validation.iterableGenericParameters.BookInfoSpec[tags="validate-iterables",indent="0"]

<1> The violation on the 1-st index in List
<2> The violation was found in Map's key

NOTE: It is not yet supported in Groovy
