...
Once again Micronaut framework leverages JSR-330 and the link:{jeeapi}/javax/inject/Qualifier.html[Qualifier] and link:{jeeapi}/javax/inject/Named.html[Named] annotations to support this use case.
...
<4> The link:{jeeapi}/javax/inject/Named.html[jakarta.inject.Named] annotation indicates that the `V8Engine` implementation is required
