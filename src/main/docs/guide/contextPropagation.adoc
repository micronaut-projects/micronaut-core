...
==== Hybrid example: scoped value first, thread local as a fallback

Sometimes you need a single element that works in both propagation modes. Implementing both interfaces lets you bind a `ScopedValue` by default and only fall back to a `ThreadLocal` when scoped values are unavailable:
...
When Micronaut runs in scoped-value mode the `ScopedValue` binding happens first, so `updateThreadContext()` returns `null` and the `ThreadLocal` branch is skipped entirely. In thread-local mode the same element transparently updates the `ThreadLocal`, meaning `currentRequestId()` continues to work without additional code.
...
=== Default Scoped Values propagation on Java 25

Micronaut Framework 5 targets Java 25 by default. Since Java 21 the JDK ships *Scoped Values* (`java.lang.ScopedValue`) as a structured alternative to classic thread locals. Because Scoped Values are available on Java 25, Micronaut enables them by default and stores propagated data in the `scoped-value` mode.

Scoped Values participate in virtual threads, enforce well-defined lifetimes, and prevent the accidental leaks that long-running thread pools often suffer when `ThreadLocal` is used directly. Unless you override the propagation mode, every piece of state you add to a `PropagatedContext` will therefore flow through Scoped Values.
...
The try-with-resources pattern exists primarily for legacy integrations that require `ThreadLocal` propagation. Whenever you can, call one of the lambda-based `propagate(…)` helpers shown earlier instead—they work in both propagation modes. Only fall back to the scope API when the surrounding code expects a `ThreadLocal` to be present. Because the scope relies on thread-local state, invoking `propagate()` while the mode is still `scoped-value` throws an `IllegalStateException` instructing you to enable thread-local support.

Configure the mode globally (default: `scoped-value`) by setting `micronaut.propagation`:

[source,yaml]
----
micronaut:
  propagation: thread-local
----
...
@Context
class ThreadLocalPropagationListener implements ApplicationEventListener<ContextStartedEvent> {
...
Restoring Scoped Values later is as simple as calling `PropagatedContextConfiguration.set(PropagatedContextConfiguration.Mode.SCOPED_VALUE);`.

NOTE: Prefer the default `scoped-value` mode whenever possible. Scoped Values integrate seamlessly with virtual threads, express the intent that the state only lives within a structured scope, and eliminate the risk of leaking `ThreadLocal` data between unrelated requests.
