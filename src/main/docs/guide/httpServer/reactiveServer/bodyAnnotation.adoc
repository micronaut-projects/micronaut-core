To parse the request body, you first need to indicate to Particle the parameter which will receive the data. This is done with the api:http.annotation.Body[] annotation.
...
<3> A rx:Single[] is returned so that Particle will only emit the response once the operation completes without blocking.
