To parse the request body, you first need to indicate to Particle the parameter which will receive the data. This is done with the api:org.particleframework.http.annotation.Body[] annotation.
...
<1> The api:org.particleframework.web.router.annotation.Post[] annotation is used with a api:http.MediaType[] of `text/plain` (the default is `application/json`.
...
TIP: The `particle.server.maxRequestSize` setting in `application.yml` will limit the size of the data (the default maximum request size is 10MB) if no `@Size` constraint is specified.
...
<2> The `collect` method is used to accumulate the data in this simulated example, but it could for example write the data to logging service, database or whatever chunk by chunk
<3> A `Flowable` is returned so that Particle will only emit the response once the operation completes without blocking.
