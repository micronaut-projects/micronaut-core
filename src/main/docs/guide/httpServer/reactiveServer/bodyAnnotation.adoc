...
<1> In this case the method is altered to receive and return an Reactor `Flux` type
<2> A reactor:Mono[] is returned so Micronaut only emits the response once the operation completes without blocking.
