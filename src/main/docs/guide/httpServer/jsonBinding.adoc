...
A api:json.JsonMapper[] can be injected into your code, and using `JsonMapper.createDefault()` it can read and write values, including  api:json.tree.JsonNode[] objects representing a json node for processing scalar or container values.
