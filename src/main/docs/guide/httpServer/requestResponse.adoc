...
<1> The factory methods of the api:org.particleframework.http.HttpResponse[] are statically imported
<2> The method is mapped to the URI `/hello` and accepts a api:org.particleframework.http.HttpRequest[]
<3> The api:org.particleframework.http.HttpRequest[] is used to obtain the value of a query parameter called `name`.
<4> The api:org.particleframework.http.HttpResponse.ok(T)[] method is used to return a api:org.particleframework.http.MutableHttpResponse[] with a text body. A header called `X-My-Header` is also added to the response object.