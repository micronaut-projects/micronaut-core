...
IMPORTANT: Note, that api:io.micronaut.http.HttpRequest[] should be declared parametrized with concrete genereic type if request body is needed, e.g. ```java
 @Get("/hello")
 public  get(HttpRequest<MyClass> request) {
    request.getBody.map(this::process);
  ...
 }```
 api:io.micronaut.http.HttpRequest.getBody()[] will be empty otherwise, event if request has a body.
