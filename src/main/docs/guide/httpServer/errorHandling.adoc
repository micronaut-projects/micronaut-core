...
Note that the previous example will only handle errors thrown from the declaring controller. To handle errors globally with the api:web.router.annotation.Error[] annotation:
...
<1> The api:web.router.annotation.Error[] is used to declare the method a global error handler
...
TIP: You can also specify an exception type to the api:web.router.annotation.Error[] to add a more specific exception handler
...
<1> The api:web.router.annotation.Error[] declares which api:http.HttpStatus[] error code to handle (in this case 404)
