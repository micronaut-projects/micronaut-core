...
Note that the previous example will only handle errors thrown from the declaring controller. To handle errors globally you should use the api:http.annotation.Error[] annotation with the `global` attribute set to `true`:
