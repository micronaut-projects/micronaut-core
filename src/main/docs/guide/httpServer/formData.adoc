In order to make data binding model customizations consistent between form data and JSON, Particle uses Jackson to implement binding data from form submissions.
...
As you can see from the example above, this approach allows you to use features as support for jdk:java.util.Optional[] types and restrict the parameters to be bound (When using POJOs you must be careful to use Jackson annotations to exclude properties that should not be bound).