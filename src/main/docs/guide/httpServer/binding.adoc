...
Also, in order to bind all request URI variables or request parameters to a command object, you can set the flag `(all = true)` in `QueryValue` as:
...
include::{testsuite}/server/binding/BookmarkController[tags=class, indent=0]

include::{testsuite}/server/binding/BookmarkController[tags=listBooks, indent=0]
----
...
|`@QueryValue String myParam` or `QueryValue(all = true) Pojo pojo`
|===
