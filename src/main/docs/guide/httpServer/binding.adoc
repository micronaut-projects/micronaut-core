...
The supported values are: `"CSV"`, `"SSV"`, `"PIPES"`, `"MULTI"` and `"DEEP_OBJECT"`, where the meaning is similar to https://swagger.io/docs/specification/serialization/[Open API v3] query parameter's `style` attribute.
...
|===
| Format | Iterable example | Map example | Object example
