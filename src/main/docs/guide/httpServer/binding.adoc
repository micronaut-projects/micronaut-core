...
Micronaut also supports binding the body to an `InputStream`. Note that in this case, it is required for the controller method execution to be offloaded to a different thread pool.
