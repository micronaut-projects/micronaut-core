...
. Immutable Bean class with getters and all argument constructor (or `@Creator` annotation on a constructor or static method).
...
. if the request can have a body and no `@Body` is defined then try parse the body (either JSON or form data) and
