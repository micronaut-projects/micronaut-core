...
If requirements dictate you must have knowledge about the metadata of the file being received, a special class called api:http.server.netty.multipart.CompletedFileUpload[] has been created that has methods to retrieve the data of the file, but also has file information like the content type and file name.
...
* api:http.server.netty.multipart.CompletedFileUpload[]
