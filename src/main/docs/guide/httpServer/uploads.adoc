...
WARNING: Note that the `getBytes` and `getByteBuffer` methods of api:http.multipart.StreamingFileUpload[] can only be called *once* per request.
...
WARNING: Note that the `getBytes` and `getByteBuffer` methods of api:http.server.netty.multipart.CompletedFileUpload[] can only be called *once* per request.
