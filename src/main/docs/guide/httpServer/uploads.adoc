Handling of file uploads has special treatment in Micronaut. Support is provided for both streaming of data through non-blocking file uploads, as well as blocking uploads which resolve when the file upload is completed.
...
To receive files in a blocking manner, so that the route is resolved only after the entire file is available, you can use the api:http.server.netty.multipart.CompletedFileUpload[] class:
