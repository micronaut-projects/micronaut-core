It is easy to validate incoming data into Micronaut' controllers with the
...
We can validate parameters using `javax.validation` annotations and the api:validation.Validated[] annotation at class level.
...
The validation behaviour is shown in the next test:
...
The validation of POJOs is shown in the next test:
