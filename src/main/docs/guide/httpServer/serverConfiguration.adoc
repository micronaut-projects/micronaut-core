...
Netty beans are enabled by default. Set `netty.enabled=false` to disable all Netty integration, or `micronaut.server.netty.enabled=false` to disable only the Netty server. These properties back api:http.netty.RequiresNetty[] and api:http.server.netty.RequiresNettyServer[].
