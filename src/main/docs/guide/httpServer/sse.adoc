...
[source,java]
----
include::{testsuite}/server/sse/Headline.java[tags=imports, indent=0]

include::{testsuite}/server/sse/Headline.java[tags=class, indent=0]
----
...
[source,java]
----
include::{testsuite}/server/sse/HeadlineController.java[tags=imports, indent=0]

include::{testsuite}/server/sse/HeadlineController.java[tags=class, indent=0]
----

<1> The controller method returns a rs:Publisher[] of api:http.sse.Event[]
<2> For each version of Particle a headline is emitted
...
<5> he rx:Emitter[] interface's `onComplete` method is used to indicate when to finish sending server sent events.
...
[source,json]
----
 data: {"title":"Particle 1.0 Released","description":"Come and get it"}
 data: {"title":"Particle 2.0 Released","description":"Come and get it"}
----
