...
NOTE: For illustration purposes we use `blockingFirst()` to obtain the client. It is however possible to combine `connect` (which returns a rx:Flowable[]) to perform non-blocking interaction via WebSocket.
