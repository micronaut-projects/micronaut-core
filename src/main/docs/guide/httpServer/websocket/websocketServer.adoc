...
* A `byte[]`, a Netty `ByteBuf` or a Micronaut api:core.io.buffer.ByteBuffer[]. A `ByteBuf` or `ByteBuffer` parameter is a view of the received frame content, which is released once the `@OnMessage` method (and any publisher it returns) has completed and the api:websocket.event.WebSocketMessageProcessedEvent[] listeners have run. Call `retain()` or copy the data if you need it beyond that point. In particular, api:websocket.WebSocketSession[] `send` takes ownership of the buffer it is given, so to echo the received buffer pass it as `session.send(buffer.retain())`.
