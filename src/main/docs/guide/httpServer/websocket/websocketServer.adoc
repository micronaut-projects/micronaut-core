...
* A `byte[]`, a `ByteBuf` or a Java NIO `ByteBuffer`. A `ByteBuf` (or api:core.io.buffer.ByteBuffer[]) parameter is a view of the received frame content and is released once the `@OnMessage` method (and any publisher it returns) completes; call `retain()` or copy the data if you need it beyond that point.
