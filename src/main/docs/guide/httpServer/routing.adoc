...
.build.gradle
----
annotationProcessor "io.micronaut:validation" // for Java only
compile "io.micronaut:validation"
----

NOTE: For Java this `validation` dependency will also need to be added to the `annotationProcessor` scope and for Kotlin the dependency will need to be added to to `kapt` scope.
