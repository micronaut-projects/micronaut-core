...
<2> The link:{api}/org/particleframework/web/router/annotation/Get.html[Get] annotation is used to map the method to an HTTP link:{api}/org/particleframework/http/HttpMethod.html#GET[GET] with a URI variable embedded in the URI called `number`
<3> The value of the URI variable is referenced in the implementation

Micronaut will map the URI `/issues/issue/{number}` for the above controller. We can assert this is the case by writing a unit test, this time in Java and JUnit:
...
<2> The tests sends a request to the URI `/issues/show/12`
<3> And then asserts the response is "Issue # 12"

If you invoke the previous endpoint without the required URI variable or with an invalid type, Micronaut responds with the appropriate HTTP failure codes as illustrated in the next Spock test.

.Testing Missing URI Variables
[source,java]
----
include::{testsuite}/server/routes/IssuesControllerSpec.groovy[indent=0]
----

=== Routing Annotations

The previous example used the link:{api}/org/particleframework/http/annotation/Get.html[@Get] annotation to add method that accepted HTTP link:{api}/org/particleframework/http/HttpMethod.html#GET[GET] requests. The following tables summarizes the available annotations and how they map to the different HTTP methods:
...
|link:{api}/org/particleframework/http/annotation/Delete.html[@Delete]
|link:{api}/org/particleframework/http/HttpMethod.html#DELETE[DELETE]

|link:{api}/org/particleframework/http/annotation/Get.html[@Get]
|link:{api}/org/particleframework/http/HttpMethod.html#GET[GET]

|link:{api}/org/particleframework/http/annotation/Head.html[@Head]
|link:{api}/org/particleframework/http/HttpMethod.html#HEAD[HEAD]

|link:{api}/org/particleframework/http/annotation/Options.html[@Options]
|link:{api}/org/particleframework/http/HttpMethod.html#OPTIONS[OPTIONS]

|link:{api}/org/particleframework/http/annotation/Patch.html[@Patch]
|link:{api}/org/particleframework/http/HttpMethod.html#PATCH[PATCH]

|link:{api}/org/particleframework/http/annotation/Put.html[@Put]
|link:{api}/org/particleframework/http/HttpMethod.html#PUT[PUT]

|link:{api}/org/particleframework/http/annotation/Post.html[@Post]
|link:{api}/org/particleframework/http/HttpMethod.html#POST[POST]

|link:{api}/org/particleframework/http/annotation/Trace.html[@Trace]
...
If you prefer to not use annotations and declare all of your routes in code then never fear, Micronaut has a flexible link:{api}/org/particleframework/web/router/RouteBuilder.html[RouteBuilder] API that makes it a breeze to define routes programmatically.
