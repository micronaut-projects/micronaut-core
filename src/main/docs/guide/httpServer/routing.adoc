...
In most cases developer has to deal only with standard http methods, such as GET, POST and etc. However, there are cases when one has to provide additional methods like REPORT or LOCK (see  https://tools.ietf.org/html/rfc4918[RFC-4918 Webdav] as one of many examples). In that case you can use @CustomHttpMethod annotation.
...
You can use this annotation in controllers, as well as declarative clients, if you need to access an application that works with extension of HTTP.
