...
A method argument of type api:http.PathVariables[] gives all variables of the matched route, converted on access like `@PathVariable` arguments, e.g. `pathVariables.getLong("id")`, `pathVariables.getInt("page", 0)` or `pathVariables.getList("ids", Long.class)`. A missing variable or a value that does not convert is answered with `400`. The request filter methods of a `@ServerFilter` that run after routing can declare it too, but not `@PreMatching` ones.
...
[.lang-python]
--
Python applications declare routes as module-level functions instead of subclassing a route builder. See <<moduleScripts, Module Scripts>> for how a module of route functions is turned into a controller.
--
