...
Micronaut supports validating route arguments at compile time with the validation library. To get started simply add a dependency.
...
* {blank}
 The URI template contains a variable that is optional, but the parameter is not annotated with @Nullable or is an Optional.
...
* {blank}
 The URI template contains a variable that is missing from the route arguments.

If an issue or bug is encountered with the route validation, please link:{github}/issues[file an issue]. In the meantime, to allow you to continue working without working around the issue, a system property can be set to disable the validation check.

Set the system property `-Dmicronaut.route.validation=false` to skip the route validation.