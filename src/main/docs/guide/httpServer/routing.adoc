...
URI variables can be referenced via method arguments. When the path variable matches the method argument name, they are bound together automatically. If you want to use different names the link:{api}/io/micronaut/http/annotation/PathVariable.html[PathVariable] annotation can be used. The following example illustrates these options:
