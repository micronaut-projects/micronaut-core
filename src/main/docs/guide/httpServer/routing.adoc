...
Particle will map the URI `/issues/issue/{number}` for the above controller. We can assert this is the case by writing a unit test, this time in Java and JUnit:
...
If you invoke the previous endpoint without the required URI variable or with an invalid type, Particle responds with the appropriate HTTP failure codes as illustrated in the next Spock test.
