...
Micronaut's HTTP server supports writing data with blocking simply by returning a rs:Publisher[] the emits objects that can be encoded to the HTTP response.
...
[source,java]
----
include::{testshttpservernetty}/writable/TemplateController.java[tags=imports, indent=0]

include::{testshttpservernetty}/writable/TemplateController.java[tags=clazz, indent=0]
----
