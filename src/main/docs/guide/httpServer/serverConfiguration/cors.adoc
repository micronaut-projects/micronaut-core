...
The `@CrossOrigin` annotation maps with a one-to-one correspondence to application-wide link:{api}/io/micronaut/http/server/cors/CorsOriginConfiguration.html[CorsOriginConfiguration] configuration properties. To specify just an allowed origin, use `@CrossOrigin("https://foo.com")`. To specify additional configuration details,
...
----
For this example, the controller class is annotated with `@CrossOrigin`, so the CORS configuration applies to all endpoints in the controller, but not to other routes in the application that don't otherwise have applicable CORS configuration.
...
----
For the following example, the `@CrossOrigon` annotation is applied more specifically to the
`GET /hello` endpoint, but not to `GET /hello/nocors`
