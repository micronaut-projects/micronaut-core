...
Consider a hypothetical use case whereby you wish to trace each request to the Particle "Hello World" example using some external system. The external system could be a database, a distributed tracing service and may require I/O operations.
...
[source,java]
----
include::{testsuite}/server/filters/TraceService.java[tags=imports, indent=0]
include::{testsuite}/server/filters/TraceService.java[tags=class, indent=0]
----
...
[source,java]
----
include::{testsuite}/server/filters/TraceFilter.java[tags=imports, indent=0]

include::{testsuite}/server/filters/TraceFilter.java[tags=class, indent=0]
    ...
}
----
...
[source,java]
----
include::{testsuite}/server/filters/TraceFilter.java[tags=doFilter, indent=0]
----
...
<3> Finally, RxJava's `doAfterNext` method is used to add a header called `X-Trace-Enabled` to the response.