...
Normally JSON parsing only happens if the content type is `application/json`. Message body handlers behave similarly in that they declare the media types they can process. To extend the list of media types that a given handler processes, provide configuration that will be stored in api:io.micronaut.http.body.config.MessageBodyConfiguration[]:
