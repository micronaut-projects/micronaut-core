...
<3> Default values can be assigned with ann:core.bind.annotation.Bindable[]
<4> Nested configuration types can provide nested configuration
<5> Optional configuration values can use language-specific nullable or optional types; Scala can use explicit-null union types
