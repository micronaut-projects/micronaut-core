...
<2> The `@Argument` annotation can be used to inject the name of the sub-property that defines the name of the bean (and the bean qualifier)
<3> Each property of the bean is bound to configuration.

The above `DataSourceConfiguration` defines a `url` property to configure one or many hypothetical data sources of some sort. The URLs themselves can be configured using any of the link:{api}/org/particleframework/context/env/PropertySource.html[PropertySource] instances evaluated to Micronaut:
