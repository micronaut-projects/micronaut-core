...
TIP: `.properties`, `.json`, `.yml` and `.env` are supported out of the box. For Groovy users `.groovy` is supported as well.
...
==== Loading Properties from .env Files

Micronaut supports loading configuration from `.env` files. Place `.env` files in `src/main/resources/`.
...
* Escape sequences (`\n`, `\t`, `\r`) in double quotes
* Variable substitution with `${VAR}` or `$VAR`
* Multiline values in quotes

Keys are normalized to Micronaut's property format (e.g., `DATABASE_URL` becomes `database.url`).

Environment-specific files (`.env.dev`, `.env.prod`) are also supported and override the base `.env` file.
