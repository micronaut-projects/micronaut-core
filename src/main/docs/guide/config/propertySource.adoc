...
When the application detects that it is running inside Kubernetes (the `kubernetes` environment is active, which the Micronaut framework deduces automatically whenever the `KUBERNETES_SERVICE_HOST` environment variable is present), OS environment variables are loaded through api:io.micronaut.context.env.KubernetesEnvironmentPropertySource[] instead of the regular loader used above.

To avoid slow application startup in large clusters -- where Kubernetes can inject thousands of https://kubernetes.io/docs/concepts/services-networking/service/#environment-variables[service link] environment variables for every `Service` in the namespace -- this property source silently excludes any environment variable whose name ends with one of the following suffixes:
...
It also excludes any variable ending in `_PORT` whose value starts with `tcp://` (the Docker-link style format Kubernetes also injects).

IMPORTANT: This filtering is based purely on the variable *name*, so it also applies to variables you define yourself, not only to the ones Kubernetes injects automatically. For example, an environment variable you set named `MY_SYSTEM_SERVICE_PORT` is silently excluded because it ends with `_SERVICE_PORT`, even though it has nothing to do with Kubernetes service discovery. Prefer a different suffix for your own application-specific environment variables to avoid this.

TIP: If your application does not rely on Kubernetes' automatic service link variables, set `enableServiceLinks: false` on the container spec in your Pod/Deployment manifest. This stops Kubernetes from injecting them in the first place, reducing the number of environment variables Micronaut has to filter through at startup. Note this does not by itself prevent the naming collision described above for a variable you define yourself; it only stops Kubernetes from adding more variables of that shape for other `Service` resources in the namespace. See the Kubernetes documentation on https://kubernetes.io/docs/concepts/services-networking/service/#environment-variables[Environment Variables] for details.

If your deployment needs every OS environment variable to be processed as configuration, including ones matching the suffixes above, disable environment deduction entirely (see <<disablingEnvironmentDetection,Disabling Environment Detection>>) so that the `kubernetes` environment is never activated and the regular, non-filtering property source is used instead. Note that this also disables automatic detection of other environments, such as `test` or `cloud`.
...
[configuration,title="Importing additional configuration"]
----
micronaut:
  config:
    import:
...
Without `optional:`, an api:io.micronaut.context.exceptions.ConfigurationException[] is thrown if the import cannot be loaded.
...
* `classpath://` throws a api:io.micronaut.context.exceptions.ConfigurationException[] and reports all matching locations.
...
snippet::io.micronaut.docs.config.importer.DemoPropertySourceImporter[tags="class",indent=0,title="Custom importer implementation"]

snippet::io.micronaut.docs.config.importer.DemoPropertySourceImporterTest[tags="test",indent=0,title="Custom importer test"]
...
If your importer loads remote configuration, api:discovery.config.RetryablePropertySourceImporter[] in `micronaut-discovery-core` provides a reusable base class that standardizes retry behavior across both connection-string and map-based imports. It parses the same retry properties from either form and applies them with Micronaut's programmatic retry support.

Standard retry properties supported by api:discovery.config.RetryablePropertySourceImporter[] are:
...
[[duplicateConfigurationResources]]
...
The behavior can be customized using the api:io.micronaut.context.ApplicationContextBuilder[] (including `Micronaut`).

snippet::io.micronaut.docs.context.env.ConfigurationLoadStrategySnippet[tags="firstMatch",indent=0,title="Using FIRST_MATCH"]

To merge duplicates, set the strategy type to `MERGE_ALL`:

snippet::io.micronaut.docs.context.env.ConfigurationLoadStrategySnippet[tags="mergeAll",indent=0,title="Merging duplicates"]

When using `MERGE_ALL`, you can optionally specify a merge order based on artifact (JAR) name patterns:

snippet::io.micronaut.docs.context.env.ConfigurationLoadStrategySnippet[tags="mergeOrder",indent=0,title="MERGE_ALL with mergeOrder"]

NOTE: `mergeOrder` is only supported when the strategy type is `MERGE_ALL`.
