...
=== Validating Properties on Startup

By default, Micronaut beans are not created on startup, but as they are needed. Any properties associated with the bean are not loaded until this point, so if a property is not defined, you will get the error at this time.

To force the validation of a bean's properties at startup, make the bean with @Context scope. By making them @Context scope the bean will be created, and the corresponding properties loaded, when the application starts.
