...
* `env://` – load key/value properties from an environment variable value
...
You can add support for a custom import protocol by implementing api:io.micronaut.context.env.PropertySourceImporter[] and registering it with Java ServiceLoader.
