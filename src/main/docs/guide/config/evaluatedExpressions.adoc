...
<5> On the `value()` member of the `@CustomAnnotation` annotation the methods of the `AnnotationMemberContext` type are made available but scoped only to the `value()` member.
