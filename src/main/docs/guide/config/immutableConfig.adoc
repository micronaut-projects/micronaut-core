...
Another way to implement immutable configuration is to define a class with constructor parameters for a ann:context.annotation.ConfigurationProperties[] or ann:context.annotation.EachProperty[] bean.
...
<2> Configuration values are bound to constructor parameters
...
The Micronaut framework prioritizes binding values from configuration for constructor parameters.
