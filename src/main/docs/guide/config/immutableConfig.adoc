...
NOTE: From a performance perspective Java records are better than interfaces; however, they have the disadvantage of not supporting link:{api}/io/micronaut/runtime/context/scope/Refreshable.html[@Refreshable]. Use one of the other mechanisms for immutable configuration if that's a requirement.
