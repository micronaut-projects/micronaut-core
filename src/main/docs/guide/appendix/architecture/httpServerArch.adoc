...
A api:http.filter.HttpServerFilter[] instance can be used by the developer to control access to server resources. By not proceeding with the api:http.filter.FilterChain[] an alternative response (such as a `403 - Forbidden`) can be returned the client barring access to sensitive resources.
