...
Once executed, if the return value is not `null` an appropriate api:http.body.MessageBodyWriter[] is selected from the api:http.body.MessageBodyHandlerRegistry[] for the response https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Content-Type[Content-Type] (defaulting to `application/json`). The message body writer encodes the return value into a `byte[]` and includes it as the body of the resulting api:http.HttpResponse[].
