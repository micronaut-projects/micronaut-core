...
One final customization option is `@Around(hotswap=true)` which which triggers the compiler to produce a compile-time proxy that implements api:aop.HotSwappableInterceptedProxy[] which defines a single method called `swap(..)` that allows swapping out the target of the proxy with a new instance (to allow this to be thread safe the generated code uses a `ReentrantReadWriteLock`).
