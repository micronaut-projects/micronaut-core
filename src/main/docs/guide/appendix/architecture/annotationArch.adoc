...
* Annotations meta-annotated with api:core.annotation.Retainable[] are, in addition to being flattened into the element's stereotypes, retained as a tree under the annotation that composed them, so an individual occurrence can be attributed to the annotation that introduced it.
