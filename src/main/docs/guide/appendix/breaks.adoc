...
error handler, Micronaut HTTP is now stricter about allowed types. In doubt, for the second body access, call
...
would be converted using `toString`, and for reading, the object would be converted using `ConversionService`. For
...
Since Micronaut 3.0 server filters are executed once per request. The `OncePerRequestHttpServerFilter` class was deprecated and marked for removal. This class is now removed.
