...
The `@Executable` methods processor api:context.processor[ExecutableMethodProcessor] should be used only for the annotations annotated with `@Executable(processOnStartup = true)` to process beans that are annotated with some annotation (executable or not) api:context.processor[BeanDefinitionProcessor] should be used.

To replicate the previous behavior of api:context.processor[ExecutableMethodProcessor] for the annotations that are not supported to be processed at the startup, it's enough to implement api:context.processor[BeanDefinitionProcessor] and iterate all the executable methods on the bean definition.

Both interfaces are revisited to have better generics and no longer have a superclass.
