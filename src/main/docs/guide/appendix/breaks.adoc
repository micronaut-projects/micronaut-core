...
=== A generated proxy no longer retains its interceptors

A proxy generated since 5.3 keeps no interceptor registrations: the non-singleton interceptors of a bean are the bean's
own, created as its dependents and destroyed with it. The deprecated `Intercepted.$interceptorRegistrations()` returns
an empty list for such a proxy. A library that read the interceptors of a bean from it reads them from the bean events
instead, `BeanEvent.getDependentBeans()`, or from `BeanRegistration.getDependentBeans()`; see
<<interceptorLifecycle, Interceptor Life Cycle>>. Proxies compiled by an earlier version still return what they
retained until they are recompiled.
