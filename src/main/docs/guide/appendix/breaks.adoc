...
A request filter method without a continuation that takes a api:http.MutableHttpRequest[] parameter receives a mutable view of the request when the request itself is not mutable, as is the case for the server request. Previously, a change of the URI of that view in place, e.g. `request.uri(URI.create("/v2/items"))` in a `void` filter method, was lost unless the method returned the view. Since Micronaut 5.3 the filter chain continues with the view when its URI was changed and the method returned no request, so the route and the filters after it see the new URI, and a api:http.server.annotation.PreMatching[] filter changes the URI the route is matched with. The view of a server request still reads the body of the original request. An asynchronous filter's URI change is taken into account when its result completes. Changes to the parameters or the body of the view are not kept unless the filter returns the view; header changes are applied to the request, as before.

When the request has no mutable view, e.g. an `HttpRequestWrapper` an earlier filter continued with, the parameter is now a mutable wrapper of it instead of failing with an `UnsupportedOperationException`.
