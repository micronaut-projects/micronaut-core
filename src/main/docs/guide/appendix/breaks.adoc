...
Code that caught `InvocationException` to unwrap the cause should catch the exception of the method instead. The change applies to classes compiled with Micronaut 5.3 or later: bean definitions compiled with an earlier version keep the previous behavior until they are recompiled.
