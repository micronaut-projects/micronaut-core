...
=== Loggers Endpoint Disabled by Default

Since logging can potentially leak sensitive information the `/loggers` endpoint is disabled by default and can be explicitly re-enabled as desired with:

.Enabling the Loggers Endpoint
[source,yaml]
----
endpoints:
    loggers:
...
Note that if it is preferable to require authentication so only use `sensitive: false` as a last resort.
