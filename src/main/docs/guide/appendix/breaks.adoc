...
=== A route variable without a regular expression is more specific (5.3)

Routes are ordered by the length of the literal parts of their URI templates, then by the number of path variables. Since Micronaut 5.3 a third key follows: among routes that tie on both, the route with fewer path variables constrained by a regular expression is more specific. For example, a `GET /items/1` request matched by both `@Get("/items/{id}")` and `@Get("/items/{id:.+}")` previously failed with a api:web.router.exceptions.DuplicateRouteException[] (answered with `400 Bad Request`), and is now routed to `/items/{id}`. A numeric modifier such as `{id:3}` limits the length of the value and is not a regular expression: it is not counted. Routes that tie on all three keys stay ambiguous.

The change is in `UriTemplateMatcher.compareTo`, and with it in the natural order (`compareTo`) of api:web.router.UriRouteInfo[] and its default implementation `DefaultUrlRouteInfo`, so the order of routes sorted with it changes too. Routes also have a new `UriRouteInfo.getOrder()` (`0` by default and for every controller route): it is consulted after the specificity of the templates, the media types and the preference of an explicit `HEAD` route over an implicit one, and only decides between routes that would otherwise be ambiguous.

=== A filter method that changes the URI of its `MutableHttpRequest` parameter changes the request (5.3)

A request filter method without a continuation can declare a api:http.MutableHttpRequest[] parameter. The server request itself cannot be changed, so the method receives a mutable copy of it: headers changed on the copy are applied to the request, but everything else only changes the copy.

Previously, changing the URI of that copy, e.g. `request.uri(URI.create("/v2/items"))` in a `void` filter method, had no effect unless the method returned the copy.

Since Micronaut 5.3, when the method changes the URI of the copy and returns nothing, the rest of the chain continues with the copy:

* the filters after it and the route see the new URI;
* for a api:http.server.annotation.PreMatching[] filter, the route is matched with the new URI;
* the body is still read from the original request;
* for a filter that returns a `Publisher` or `CompletionStage`, the URI is checked when that result completes.

Changes to the query parameters or the body of the copy are still only kept when the method returns the copy.

If an earlier filter continued with a request that cannot be copied this way, for example an `HttpRequestWrapper`, the parameter is now a mutable wrapper of that request. Previously the method failed with an `UnsupportedOperationException`.

=== A reflectively dispatched executable method propagates the method's exception

An executable method that generated code cannot call directly, for example a `private` method annotated with `@Executable` and `@ReflectiveAccess`, is dispatched through reflection. Previously `ExecutableMethod.invoke` wrapped whatever such a method threw in an api:core.reflect.exception.InvocationException[], with the `java.lang.reflect.InvocationTargetException` as its cause, while the same method declared package-private or public threw its own exception.
...
In Micronaut Framework 5, if a configuration file such as `application.properties` or `application.yml` is present more than once on the classpath, Micronaut now fails fast by default with a api:io.micronaut.context.exceptions.ConfigurationException[] describing the conflicting locations.

See <<duplicateConfigurationResources, Duplicate Configuration Resources>> for more options (including merging duplicates).

To restore the previous behavior (first match wins), configure the application context builder:

snippet::io.micronaut.docs.context.env.ConfigurationLoadStrategySnippet[tags="restoreFirstMatch",indent=0,title="Restoring FIRST_MATCH behavior"]

==== Adoption of the IANA standard for YAML media types

Micronaut now uses official **application/yaml** media type for YAML format. Before it was **application/x-yaml**.
See https://www.rfc-editor.org/rfc/rfc9512.html for details.
...
Check https://groovy-lang.org/releasenotes/groovy-5.0.html#Groovy5.0-breaking[Groovy 5 breaking changes].

==== JSpecify Nullability Annotations

The Micronaut APIs use https://jspecify.dev[JSpecify] annotations, and we recommend users to embrace JSpecify nullability annotations.
...
There are new options added to api:context.ApplicationContextBuilder[ApplicationContextBuilder] which allow modifying the behavior of the bean context without overriding it:
...
The `@Executable` methods processor api:context.processor[ExecutableMethodProcessor] should be used only to process the executable methods that should be processed at startup. The annotation should be annotated with `@Executable(processOnStartup = true)`. For other use-cases (executable or not) api:context.processor[BeanDefinitionProcessor] should be used.
...
Executable methods annotated with `@Parallel` will not trigger parallel execution of api:context.processor[ExecutableMethodProcessor]. Users who relied on that functionality should handle parallelism separately.
