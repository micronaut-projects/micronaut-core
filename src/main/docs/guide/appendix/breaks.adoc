...
The `micronaut.server.netty.redispatch-non-blocking-only` setting now controls when Micronaut switches to the configured executor:

* `true` delays redispatch until execution reaches a non-blocking thread. This reduces repeated executor hops and avoids creating additional virtual threads for every filter, event listener, and controller stage after request processing has already moved to a blocking-capable thread.
