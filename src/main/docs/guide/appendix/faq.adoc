...
Micronaut supports Scala 3 through the `micronaut-inject-scala` compiler plugin. The plugin processes typed Scala compiler trees at compile time and generates the same kind of bean definitions, executable method metadata, and bean introspections as the Java, Groovy, Kotlin, and Python language adapters.

Scala 2.x is not supported by this compiler plugin.
