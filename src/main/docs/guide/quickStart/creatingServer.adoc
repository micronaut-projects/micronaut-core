...
By default the Micronaut HTTP server is configured to run on a random port, but you can alter that be adding the following configuration to `src/main/resources/application.yml`:
...
In order to create a service that responds to "Hello World" you first need a controller. The following in an example of a controller written in Java and located in `src/main/java/example/helloworld`:
