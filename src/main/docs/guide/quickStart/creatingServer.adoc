...
(NOTE: The applications generated via our CLI come with Gradle and Maven wrappers, so its not even necessary to have Gradle or Maven installed on your machine to begin running the applications. Simply use `mvnw` or `gradlew` commands, as explained further below.)
