...
create-bean          create-client        create-controller         create-function
create-job           help
...
WARNING: Groovy automatically imports `groovy.lang.Singleton` which can be confusing as it conflicts with `javax.inject.Singleton`. Make sure you include the `javax.inject.Singleton` import when declaring a singleton bean to avoid a compilation error.
