...
4. A `native-image.properties` configuration file to `src/main/resources/META-INF/native-image` simplify building the image.
...
Micronaut includes an annotation processor that helps to handle generating the `reflection-config.json` metadata that is automatically picked up by the `nativeimage` tool:
...
@TypeHint(
        ...
        accessType = TypeHint.AccessType.CLASS_LOADING // <2>
)
----
