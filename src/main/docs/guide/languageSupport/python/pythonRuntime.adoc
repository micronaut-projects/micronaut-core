...
The GraalPy version is `{graalpyVersion}`. Use a GraalVM JDK from the matching GraalVM release line, for example GraalVM 25.4.x for GraalPy 25.4.x. The JDK's build string then contains the same Truffle line, such as `25.0.4.1.1+1-LTS-jvmci-25.4-b23`.
