...
A lookup outside the configured packages raises a Python `KeyError` for `java.type` or an `ImportError` for an import. Leave the setting unset to keep the default, unrestricted lookup.
