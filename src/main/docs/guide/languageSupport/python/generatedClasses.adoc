...
`toString()` of the generated class calls `__str__` of the Python class, or `__repr__` when the class defines no `__str__`, so Java code that formats the object sees the Python representation: a serializer writing the object as a map key, a log statement, a `text/plain` response or `String.valueOf(...)`. A class defining neither keeps the default `Object.toString()`.

snippet::micronaut.docs.tostring.Feature[tags="class", indent=0, title="The string representation of a Python class"]

<1> `String.valueOf(feature)` and the string form of a Java collection holding the object give the feature name

`__eq__` and `__hash__` are not bridged: the identity of a generated instance is that of the Java object, as for a Python class defining neither.
