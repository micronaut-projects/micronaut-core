...
* `==`, `!=` and `hash()` are those of the Python object (a dataclass compares by value), so a deserialized `Book` compares equal to a `Book` constructed in Python. It can be looked up in a set or a dictionary when the Python class is hashable: a frozen dataclass, or one declared with `unsafe_hash=True` or a `__hash__` method. A non-frozen dataclass sets `__hash__` to `None`, so its wrapper is unhashable too (`TypeError: unhashable type`), as the Python object is.
