...
* A class nested in another class extends a Java class the same way (its generated Java class is `Outer$Inner`). A class defined inside a function has no generated Java class; it extends the Java class through the GraalPy host adapter, with its rules, and is not injectable.
