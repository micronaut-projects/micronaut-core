...
A class that carries an introduction stereotype but also has state or behaviour of its own (an `__init__`, injected attributes or a concrete method next to the abstract ones) is compiled to a Java class instead: its abstract methods are implemented by the introduction advice at runtime while the rest of the class remains a Python object.
