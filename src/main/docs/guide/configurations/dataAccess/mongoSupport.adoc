...
==== Mongodb POJO mapping using Mongodb driver
The 3.5 release of mongo db reactive driver adds POJO support via the PojoCodec, which allows for direct serialization of POJOs and Java Beans to and from BSON. 
POJO support is added automatically to micronaut via the option ``PojoCodecProvider.Builder#automatic(true)``.
More details about this feature is documented on http://mongodb.github.io/mongo-java-driver/3.8/bson/pojos/[Mongodb java reactive driver].

You can find a usage example at https://github.com/micronaut-projects/micronaut-examples/tree/master/petstore/pets[micronaut-examples: petstore]. Pet Pojo declared https://github.com/micronaut-projects/micronaut-examples/blob/master/petstore/api/src/main/java/example/api/v1/Pet.java[here]
