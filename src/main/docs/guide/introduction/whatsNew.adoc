...
NOTE: You can still build Micronaut 3.2 applications or libraries using the 2.x version of the Gradle plugin. Documentation for this version can be found https://github.com/micronaut-projects/micronaut-gradle-plugin/blob/2.0.x/README.md[here].
...
It is now possible to send resources, e.g. stylesheets required by a HTML page, to the client alongside the request for the page using the HTTP2 server push protocol. Simply add a api:http.PushCapableHttpRequest[] parameter to your controller, check that `isServerPushSupported()`, and trigger the load using the `serverPush` method (for example: `request.serverPush(HttpRequest.GET("/static/style.css"))`). This can improve latency for the client, because they do not have to request that resource manually anymore, and can save a roundtrip.
