...
The Micronaut framework features dedicated support for defining both Kafka `Producer` and `Consumer` instances. Micronaut's applications built with Kafka can be deployed with or without the presence of an HTTP server.
