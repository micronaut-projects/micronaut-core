...
The Kafka key can be specified by providing a parameter annotated with `@KafkaKey`. If no such parameter is specified the record is ent with a `null` key.

The value is send it resolved by selecting the argument annotated with nn:messaging.annotation.Body[], otherwise the first argument with no specific binding annotation is used. For example:
