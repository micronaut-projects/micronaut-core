...
Netty beans are enabled by default. Set `netty.enabled=false` to disable all Netty integration, or `micronaut.http.client.netty.enabled=false` to disable only the Netty client. These properties back api:http.netty.RequiresNetty[] and api:http.client.netty.RequiresNettyClient[].
