...
Another way to create an `HttpClient` is with the static `create` method of api:http.client.RxHttpClient[], however this approach is not recommended as you must ensure you manually shutdown the client, and of course no dependency injection will occur for the created client.

==== Performing an HTTP GET

Generally there are two methods of interest when working with the `HttpClient`. The first is `retrieve`, which executes an HTTP request and returns the body in whichever type you request (by default a `String`) as an RxJava rx:Flowable[].
...
<1> The `hello` method returns a rx:Maybe[] which may or may not emit an item. If an item is not emitted, a 404 is returned.
<2> The `retrieve` method is called which returns a rx:Flowable[]. This has a `firstElement` method that returns the first emitted item or nothing
...
Consult the logging chapter for <<httplogging, howto debugging and Tracing the HTTP Client>>.
