Micronaut's HTTP client includes support for streaming data over HTTP via the api:http.client.ReactorStreamingHttpClient[] interface which includes methods specific to streaming including:
