...
You can combine many of these return types together. For example, if you wanted a reactive interface that provided access to the HTTP response headers as well as a body that was already converted to a Book, you could return `Flowable<HttpResponse<Book>>`.
