...
For example, the following binder will handle arguments passed to clients that are of the `Metadata` type:

snippet::io.micronaut.docs.http.client.bind.MetadataClientArgumentBinder[tags="clazz", indent=0, title="Typed Argument Binder"]
