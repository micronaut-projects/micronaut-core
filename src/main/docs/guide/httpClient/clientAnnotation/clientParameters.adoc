...
The `@QueryValue` annotation's format parameter allows specifying the formatting of the parameter in the request, e.g.: `@QueryValue(format=QueryValue.Formats.DEEP_OBJECT)`.
...
The following binder specifies the behavior:
...
<3> The custom `bind` method is used to implement the behavior of the binder
