...
.Locale conversion
====
A `java.util.Locale` as argument is automatically converted to an `Accept-Language` header. +
If you need the locale in your path or query and not converted, add the appropriate annotation (ann:io.micronaut.http.annotation.PathVariable[] or ann:io.micronaut.http.annotation.QueryValue[]])
====
