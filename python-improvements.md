...
context is closed before a failure, an `Error` included since S4, is rethrown with its own type); the T4 test changes the module named only inside the
...
| S4 | Low | The cleanup loops caught `RuntimeException` only, so an `Error` from one close skipped the rest. | DONE: every action runs (each in a `finally` that runs the next), and a failure propagates with its own type. |
...
| Q5 | Low | The bootstrap failure handlers caught `RuntimeException` from the cleanup close only, so an `Error` from it replaced the bootstrap failure. | DONE: the cleanup runs in a `finally` guarded by a success flag, so the bootstrap failure propagates whatever its type; a failing close is logged. |
