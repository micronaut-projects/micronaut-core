...
class NettyJsonStreamHandlerSpec extends Specification {
    def parse(Class<? extends PiecewiseMessageBodyReader<?>> readerType, Argument<?> type, String input, Object expected) {
        given:
        ...
        def actual = Flux.from(reader.readPiecewise(type, null, null, Flux.just(buf)))
                .collectList()
                .block()
        then:
        actual == expected
