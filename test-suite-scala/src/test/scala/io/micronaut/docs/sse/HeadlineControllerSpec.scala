  ...
  def testClientAnnotationStreaming(): Unit =
    ...
    try
      val headlineClient = embeddedServer
        ...
        .getBean(classOf[HeadlineClient])

      val headline: Event[Headline] = Mono.from(headlineClient.streamHeadlines()).block()

      assertNotNull(headline)
      assertTrue(headline.getData.text.startsWith("Latest Headline"))
    finally
