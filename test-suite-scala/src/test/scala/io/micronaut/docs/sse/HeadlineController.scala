...
class HeadlineController:
  ...
  // tag::streaming[]
  @Get(value = "/headlines", processes = Array(MediaType.TEXT_EVENT_STREAM)) // <1>
  def streamHeadlines(): Publisher[Event[Headline]] =
    Flux.create[Event[Headline]]((emitter: FluxSink[Event[Headline]]) => { // <2>
      val headline = Headline()
      headline.text = s"Latest Headline at ${ZonedDateTime.now()}"
