...
import io.micronaut.docs.basics.HelloControllerSpec
...
class TraceFilterMethodsSpec:
  ...
  @Test
  def testTraceFilter(): Unit =
    val server = ApplicationContext.run(
      classOf[EmbeddedServer],
      Map[String, Object](
        "spec.name" -> classOf[HelloControllerSpec].getSimpleName,
        "spec.filter" -> "TraceFilterMethodsSpec"
      ).asJava
    )
    try
      val client = server.getApplicationContext.createBean(classOf[HttpClient], server.getURL)
      try
        val response = client.toBlocking.exchange(HttpRequest.GET("/hello/John"))
