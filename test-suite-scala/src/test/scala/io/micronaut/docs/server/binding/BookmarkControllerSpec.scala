...
class BookmarkControllerSpec:
  ...
  @Test
  def testBindingPagination(): Unit =
    val server = ApplicationContext.run(
      classOf[EmbeddedServer],
      Map[String, Object]("spec.name" -> "BookmarkControllerSpec").asJava
    )
    try
      val client = server.getApplicationContext.createBean(classOf[HttpClient], server.getURL)
      try
        val template = UriTemplate("/api/bookmarks/list{?offset,max,sort,order}")
        val params = Map[String, Object](
          "offset" -> Integer.valueOf(0),
          "max" -> Integer.valueOf(10)
        ).asJava
