...
class AlertsEndpoint:
  private val alerts = CopyOnWriteArrayList[String]()

  @Read
  def getAlerts(): List[String] = alerts
  ...
  @Write(consumes = Array(MediaType.TEXT_PLAIN))
  @Sensitive(property = "add.sensitive", defaultValue = true) // <3>
  def addAlert(@Body alert: String): Unit =
    alerts.add(alert)
// end::clazz[]
