...
class BackwardCompatibleControllerSpec:
  ...
  private def withClient(test: HttpClient => Unit): Unit =
    val server = ApplicationContext.run(
      ...
      Map[String, Object]("spec.name" -> "BackwardCompatibleControllerSpec").asJava
    )
    try
  ...
  @Test
  def controllerSupportsMultipleUris(): Unit =
    withClient { client =>
      assertEquals("Hello, Fred", client.toBlocking.retrieve("/hello/Fred"))
      assertEquals("Hello, Wilma", client.toBlocking.retrieve("/hello/person/Wilma"))
    }
