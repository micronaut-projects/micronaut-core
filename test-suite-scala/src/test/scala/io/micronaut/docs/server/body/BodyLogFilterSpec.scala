...
class BodyLogFilterSpec:
  ...
  @Test
  def byteBodyCanBeSplitAndLogged(): Unit =
    val server = ApplicationContext.run(
      classOf[EmbeddedServer],
      Map[String, Object]("spec.name" -> "BodyLogFilterSpec").asJava
    )
    try
      val client = server.getApplicationContext.createBean(classOf[HttpClient], server.getURL)
      try
        val response = client.toBlocking.exchange(HttpRequest.POST("/person", Person("Fred", "Flintstone")))
