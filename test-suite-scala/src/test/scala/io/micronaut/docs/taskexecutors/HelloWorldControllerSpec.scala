...
class HelloWorldControllerSpec:
  ...
  @Test
  def executeOnBlockingControllerResponds(): Unit =
    val server = ApplicationContext.run(
      classOf[EmbeddedServer],
      Map[String, Object]("spec.name" -> "TaskExecutorsBlockingTest").asJava
    )
    try
      val client = server.getApplicationContext.createBean(classOf[HttpClient], server.getURL)
      try assertEquals("Hello World", client.toBlocking.retrieve("/hello/world"))
      finally client.stop()
    finally server.stop()
