...
class BasicAuthFilterSpec:
  ...
  @Test
  def testTheFilterIsApplied(): Unit =
    val server = ApplicationContext.run(
      classOf[EmbeddedServer],
      Map[String, Object]("spec.name" -> "BasicAuthFilterSpec").asJava
    )
    try
      val client = server.getApplicationContext.getBean(classOf[BasicAuthClient])
