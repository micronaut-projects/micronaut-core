...
import org.atinject.tck.auto.{FuelTank, Tire}
...
class SpareTire(val forSupertype: FuelTank, override val constructorInjection: FuelTank) extends Tire(forSupertype) {
  // @Inject override protected var fieldInjection: FuelTank = Tire.NEVER_INJECTED // mutable variable cannot be overridden:

  // override private[auto] var methodInjection: FuelTank = Tire.NEVER_INJECTED // mutable variable cannot be overridden:
  ...
  def getSpareTirePackagePrivateMethod3Injected: Boolean = spareTirePackagePrivateMethod3Injected
