 ...
 * out alone, and a concurrent request on the same connection still gets its response.
 */
class Http2StreamReadTimeoutTest {
    ...
    void timedOutStreamTimesOutAlone(boolean tls) throws Exception {
        try (ApplicationContext ctx = start(tls);
            ...
            CompletableFuture<HttpResponse<?>> concurrent = Mono.from(client.exchange(request(server, 500), null, null)).<HttpResponse<?>>map(r -> r).toFuture();
            ...
            // the concurrent request gets its response on the same connection
            Assertions.assertEquals(port, body(concurrent.get(10, TimeUnit.SECONDS)));

            // the timed out request was the last one of the connection, which winds down: a new
            // request gets its response on a new connection
            Assertions.assertNotNull(body(exchange(client, server, 0, null)));
        }
    }
