...
class QueryParametersSpec extends Specification {
    ...
    void "test query value with default value"() {
        given:
        ReactorHttpClient lowLevelClient = embeddedServer.getApplicationContext().createBean(ReactorHttpClient.class, embeddedServer.getURL())

        expect:
        client.searchDefault("Riverside").albums.size() == 2
        client.searchDefault(null).albums.size() == 1
        lowLevelClient.retrieve(HttpRequest.GET('/itunes/search-default'), SearchResult).blockFirst().albums.size() == 1

        when:
        lowLevelClient.retrieve(HttpRequest.GET('/itunes/search-exploded/list'), SearchResult).blockFirst()
        ...
        when:
        lowLevelClient.retrieve(HttpRequest.GET('/itunes/search-exploded/map'), SearchResult).blockFirst()
        ...
        when:
        lowLevelClient.retrieve(HttpRequest.GET('/itunes/search-exploded/pojo'), SearchResult).blockFirst()
