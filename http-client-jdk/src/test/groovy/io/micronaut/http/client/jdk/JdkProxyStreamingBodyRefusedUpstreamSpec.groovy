 ...
 */
class JdkProxyStreamingBodyRefusedUpstreamSpec extends Specification {
    ...
    void setupSpec() {
        ...
        server = ApplicationContext.run(EmbeddedServer, [
            'spec.name'                          : 'JdkProxyStreamingBodyRefusedUpstreamSpec',
    ...
    void "a chunked request proxied to a refused upstream gets the error, and the connection serves the next request"() {
        ...
        CompletableFuture<Void> written = CompletableFuture.runAsync {
            out.write(("POST /jdk-refused-proxy/relay HTTP/1.1\r\n" +
                "Host: localhost\r\n" +
        ...
        out.write("GET /jdk-refused-proxy/ping HTTP/1.1\r\nHost: localhost\r\n\r\n".getBytes(StandardCharsets.US_ASCII))
        out.flush()
    ...
    @Controller("/jdk-refused-proxy")
    @Requires(property = "spec.name", value = "JdkProxyStreamingBodyRefusedUpstreamSpec")
