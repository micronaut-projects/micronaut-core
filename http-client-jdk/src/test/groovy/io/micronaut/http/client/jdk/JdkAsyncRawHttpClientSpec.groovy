...
/**
 * Exchanges of the JDK {@link AsyncRawHttpClient}, the {@link DefaultAsyncOverRawHttpClient}
 * over a {@link JdkRawHttpClient}, however it is obtained: injected, created, from a
 * {@link RawHttpClient} or from the {@link RawHttpClientRegistry}.
 */
class JdkAsyncRawHttpClientSpec extends Specification {
    static final String SPEC_NAME = 'JdkAsyncRawHttpClientSpec'
    ...
    void "the async view of a created raw client exchanges raw bytes"() {
        given:
        RawHttpClient rawClient = RawHttpClient.create(server.URI)
    ...
    void "the Host header is computed from the URI"() {
        ...
        when:
        ByteBodyHttpResponse<?> response = send(client, HttpRequest.GET(uri).header(HttpHeaders.HOST, "gateway.example"), RawRequestOptions.proxy())

        then:
        text(response) == uri.host + ":" + uri.port

        cleanup:
        response?.close()
    }

    void "retaining the Host header fails the stage unless the JDK allows the header, and releases the request body"() {
        given:
        ...
        when:
        client.exchange(HttpRequest.POST(server.URI.toString() + "/async-raw/echo", null).header(HttpHeaders.HOST, "gateway.example"), body,
        ...
        e.cause.message.contains("jdk.httpclient.allowRestrictedHeaders")
        closed.get()
    }

    void "an encoded response is not decompressed"() {
        given:
        AsyncRawHttpClient client = server.applicationContext.getBean(AsyncRawHttpClient)

        when:
        ByteBodyHttpResponse<?> response = send(client, HttpRequest.GET(server.URI.toString() + "/async-raw/gzip"), RawRequestOptions.proxy())

        then:
        response.headers.get(HttpHeaders.CONTENT_ENCODING) == "gzip"
        response.byteBody().buffer().get().toByteArray() == GZIPPED
    ...
    void "the raw client sends an exchange with options through the publisher API"() {
        given:
        RawHttpClient rawClient = server.applicationContext.createBean(RawHttpClient)
    ...
    void "a request without a body is sent"() {
        given:
        AsyncRawHttpClient client = server.applicationContext.getBean(AsyncRawHttpClient)

        when:
        ByteBodyHttpResponse<?> response = exchange(client, HttpRequest.GET(server.URI.toString() + "/async-raw/target"), null, null)
    ...
    void "closing an unconsumed response closes the connection"() {
        given:
        RawSocketUpstream upstream = new RawSocketUpstream()
        AsyncRawHttpClient client = server.applicationContext.getBean(AsyncRawHttpClient)
        ...
        then: 'the JDK client closes the connection of a response whose body was not read'
        connection.awaitClosed(TIMEOUT_SECONDS)
