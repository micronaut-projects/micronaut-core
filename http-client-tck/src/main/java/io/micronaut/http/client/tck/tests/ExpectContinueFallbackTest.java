 ...
 * sends its body after the configured time anyway, instead of waiting for the read timeout.
 */
@SuppressWarnings({
...
class ExpectContinueFallbackTest {
    ...
    void bodyIsSentWhenTheServerIgnoresTheExpectation() throws Exception {
        try (RawUpstream upstream = new RawUpstream();
            ...
            CloseableByteBody body = ByteBodyFactory.createDefault(ByteArrayBufferFactory.INSTANCE).adapt(chunks);
            MutableHttpRequest<?> request = HttpRequest.create(HttpMethod.POST, upstream.uri("/expect").toString())
                .header(HttpHeaders.EXPECT, "100-continue")
                .contentType(MediaType.TEXT_PLAIN_TYPE);

            long start = System.nanoTime();
            CompletableFuture<HttpResponse<?>> pending = Mono.<HttpResponse<?>>from(
                client.exchange(request, body, null, RawRequestOptions.proxy())).toFuture();
