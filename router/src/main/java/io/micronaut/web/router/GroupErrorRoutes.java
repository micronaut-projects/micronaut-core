 ...
 * it, and for a located route, then in the groups of the locator routes that located it, the
 * closest locator first. Within a group, the error route of the closest exception type answers.
...
public final class GroupErrorRoutes {
    ...
    public static <R> @Nullable RouteMatch<R> findErrorRoute(HttpRequest<?> request, @Nullable RouteInfo<?> routeInfo, Throwable error) {
        ...
        for (RouteAssembly.RouteGroup scope : locatorScopes(request, route)) {
    ...
    /**
     * The groups of the locator routes that located the matched route, the closest locator first.
     */
    private static List<RouteAssembly.RouteGroup> locatorScopes(HttpRequest<?> request, DefaultUrlRouteInfo<?, ?> route) {
        ...
        if (match instanceof DefaultUriRouteMatch<?, ?> uriMatch
            ...
            && uriMatch.matchInfo() instanceof RouteLocator.LocatedUriMatchInfo located) {
            return located.errorScopes();
        }
        return List.of();
    }
}
