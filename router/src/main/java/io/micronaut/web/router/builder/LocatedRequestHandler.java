...
/**
 * A route handler of the route table of a located target that receives the target, see
 * {@link LocatedHttpRouteBuilder}: otherwise the same as a {@link RequestHandler}.
 *
 * <pre>{@code
 ...
 * @since 5.3.0
 * @see LocatedHttpRouteBuilder#handle(io.micronaut.http.HttpMethod, String, LocatedRequestHandler)
 */
@Experimental
@FunctionalInterface
public interface LocatedRequestHandler<T> {
     ...
     */
    HttpResponse<?> handle(HttpRequest<?> request, PathVariables pathVariables, T target) throws Exception;
}
