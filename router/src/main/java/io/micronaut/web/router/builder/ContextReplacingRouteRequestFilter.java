...
/**
 * A filter of one route's requests that changes the propagated context and can answer or replace
 * the request, declared with
 * {@link HttpRouteSpec#beforeReplacing(ContextReplacingRouteRequestFilter)}: the {@link ReplacingRouteRequestFilter} that
...
@Experimental
@FunctionalInterface
public interface ContextReplacingRouteRequestFilter {

    /**
     * Filter the request.
     *
     * @param request           The request, to change in place for what runs after the filter, see {@link ReplacingRouteRequestFilter}
