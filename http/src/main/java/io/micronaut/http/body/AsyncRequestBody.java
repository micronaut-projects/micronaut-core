 ...
 * }</pre>
 *
 * <p>What a read left open, e.g. the parts of a form that were not read, is released when the
 * request ends.</p>
 *
 * <h2>One read</h2>
 ...
 * the handler's read. It buffers the body before the handler can read it. {@link #form()} is the
 * one form of the request, shared with the {@code FormData}, {@code FileUpload} and text field
 * arguments of the filters and of the route: a filter that reads the form leaves it readable.</p>
 *
...
public interface AsyncRequestBody {
     ...
     * @param type The type of an element
     * @param <T>  The type of an element
     * @return The elements, read as they are asked for. They are closed when the request ends
     * @throws IllegalStateException    if the body was already read
     * @throws IllegalArgumentException if the type of an element is a reactive or asynchronous
    ...
    /**
     * Read the submitted form part by part, as it arrives. The parts are closed when the request
     * ends.
     *
