 ...
 * to the file upload to obtain the final completed upload.</p>
 * <p>
 * <p>Note that if {@link #isComplete()} is false and this is a partial file upload then the various methods to obtain
 * the data within the upload return only the data for the current chunk.</p>
 * <p>
 * <p>The {@link #transferTo(String)} method can be used whether the upload is complete or not. If it is not complete
 * the framework will automatically subscribe to the upload and transfer the data chunk by chunk in a non-blocking manner</p>
 * <p>
