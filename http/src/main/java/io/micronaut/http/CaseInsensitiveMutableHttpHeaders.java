 ...
 */
@Internal
@SuppressWarnings("InnerTypeLast") // Keeping the Netty code separate from the Micronaut code
public class CaseInsensitiveMutableHttpHeaders implements MutableHttpHeaders {
     ...
     * @param conversionService The conversion service
     */
    public CaseInsensitiveMutableHttpHeaders(Map<String, String> defaults, ConversionService conversionService) {
    ...
    public List<String> getAll(CharSequence name) {
        return backing.get(name.toString());
    }
    ...
    private void validate(CharSequence header, CharSequence value) {
        ...
        if (index != -1) {
            throw new IllegalArgumentException("a header name can only contain \"token\" characters, " +
                "but found invalid character 0x" + Integer.toHexString(header.charAt(index)) +
                " at index " + index + " of header '" + header + "'.");
        }
