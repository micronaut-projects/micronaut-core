...
class HttpRequestSpec extends Specification {
    ...
    def "Converting to url and getting host and authority work as expected"() {
        ...
        verifyAll {
            ...
            assert HttpRequest.GET(new URI("https://slave1-6x8-build-agent-2.0.1-5h7sl:8080")).getUri().getAuthority() == "slave1-6x8-build-agent-2.0.1-5h7sl:8080"
            assert HttpRequest.GET(new URI("https://slave1-6x8-build-agent-2.0.1-5h7sl")).getUri().getAuthority() == "slave1-6x8-build-agent-2.0.1-5h7sl"
        }
    }
}
